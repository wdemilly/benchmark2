"""
Micro-Prompt Harness — Quality Floor + Scanner-Ranked Pipeline + Stage F
========================================================================
...
def run_pipeline(
    ...
    # Runs on the final_text. Does not gate shipping. The predicted score is
    # written to the summary alongside the recommendation band. If the labeled
    # corpus is missing or too small, Stage F reports UNAVAILABLE and the rest
    # of the pipeline is unchanged.
    try:
        predictor = stage_f_load_predictor(str(LABELED_CORPUS_PATH), STAGE_F_RIDGE_LAMBDA)
        result["stage_f"] = stage_f_predict(result.get("final_text", ""), predictor)
    except Exception as e:
        result["stage_f"] = {
            "available": False,
