...
v32 adds nine new pattern detectors targeting cadences observed in the
Originality orange band on the v31 export: triple noun-fragment
inventories without eval heads (Class A), three-adjective stacks with two
internal commas (Class B), relaxed anaphora — 2-element with shared 3-word
opener and 3-element with broken-third opener (Class C), summative short
closer following a long sentence (Class D), stacked which-was relative
clauses (Class E), hedged noun-appositive (Class F, piggybacks on
SCENE_SET marker chain as marker F), single-sentence body-part inventory
(Class G), lyrical participial cascade (Class H), and tense-shift outliers
(Class I). Six of the nine are trim-able (A, B, D, E, F, G) — the trim
strategy deletes the over-specified element and repairs punctuation, with
no LLM call required. The other three (C, H, I) are GRAFT_ONLY: detection
flags the sentence, G3a still attempts a same-beat graft, G3c falls
through to manual flag if no clean graft exists. Trim entries live in
AI_TELL_WORDS with callable replacements; apply_deletion_heuristic
already accepts callables via re.sub. Donor-rejection in
_donor_sentence_is_clean is extended with the v32 patterns so the grafter
cannot pull violations back in. multi_sent_rules gains entries for the
classes whose flag spans multiple sentences (C, D).
v33 replaces v32's absolute-zero hard-cap logic with density-relative
gating against a human reference corpus (Jaymin Eve / Shadow Beast
Shifters, ~8800 words, same paranormal-romance register). Per-pattern
density per 1000 words is computed for each draft and compared to the
corpus baseline. A pattern fails the cap only when its density exceeds
2.0× the corpus mean (with a small absolute floor of 0.5/1k for
patterns where the corpus density is zero, to allow rare incidental
hits). The diagnostic that motivated this: v32 hard-capped at zero
hits for many patterns the human corpus also produces — including
participle_cascade (corpus 1.37/1k, AI 1.18/1k), triple_fragment
(corpus 1.59/1k, AI 1.18/1k), and mid_appositive (corpus 1.14/1k,
AI 0.39/1k). All three were flagging cadences below human baseline.
v33 also inverts the em-dash rule. The corpus uses em-dashes at
3.08/1k; the v32 chapter used zero. v31's em-dash cap was actively
suppressing a normal human cadence. v33 enforces a *floor* (≥ 1.5/1k)
and removes any cap. Below the floor, the chapter fails and regenerates
with the prompt instructed to use em-dashes.
v33 adds a sentence-length distribution check. The corpus has a
roughly centered distribution (mean 15.3, median 15, stdev 9.0,
short-rate 14%, long-rate 6%). The v32 chapter was U-shaped (median 9,
short-rate 26%, long-rate 10%). v33 fails chapters where the median
is below 70% of corpus median, OR very-short rate is above 1.5× corpus,
OR very-long rate is above 2.0× corpus. The corpus baseline lives in
CORPUS_BASELINE_V33 below; it can be regenerated from any reference
text by running the density computation in scan_draft against it.
v34 adds the prompt-side companion to v33's scanner gates. The v33
em-dash floor and distribution check fail chapters that miss em-dashes
or skew short, but the model had no instruction to deploy em-dashes
or hold the long-sentence end of the distribution open during
generation. The first v33 run produced a chapter with zero em-dashes
(floor required 4) and a 34% short-sentence rate (cap was 21%) and
shipped anyway because the gate exhausted retries. v34 augments
SYSTEM_PROMPT with six rules: (1) em-dash floor and target rate, with
a comma-cluster-to-em-dash transformation example; (2) passage pattern
budget — at most one rhythmic device per 60-word window, addressing
the stacking failure mode v33 architecture cannot see; (3) past-perfect
limit, prohibiting chains and ritualized "the day a man had stood"
flashback constructions; (4) templated emotion echo prohibition,
banning "I was X. Plain X..." and "I was X. That was the truth..."
two-sentence shapes; (5) repeated short subject-verb fragment limit,
banning more than one "She paced." per chapter; (6) long-sentence
variance, requiring 5-8% of sentences at 30+ words built from
structural means rather than figurative reach. The v34 rules apply
globally regardless of which outline is loaded; they are universal
AI-detection counter-instructions for the paranormal-romance register
calibrated to the same Eve corpus baseline embedded in v33.
...
GRAFT_ONLY = None
# ============================================================================
# v33 — Human reference corpus baseline (Jaymin Eve / Shadow Beast Shifters,
# ~8800 words). Per-pattern densities per 1000 words and sentence-length
# distribution. Used by hard_cap_pass to gate drafts on density-relative
# thresholds rather than absolute-zero hits.
#
# To regenerate: run scan_draft against any reference text and record the
# scan_<X>_count fields divided by (word_count / 1000). Update both blocks
# below. The thresholds (DENSITY_FAIL_MULTIPLIER, etc.) are tuned for the
# current corpus; if you swap in a much larger or different-genre corpus,
# revisit them.
# ============================================================================
CORPUS_BASELINE_V33 = {
    "corpus_word_count": 8779,
    "corpus_source": "Jaymin Eve, Rejected: Shadow Beast Shifters #1, ~8 chapter excerpts",
    "densities_per_1k": {
        "the_way": 0.57,
        "periphrastic": 0.0,
        "not_but": 0.23,
        ...
        "backfill": 0.0,
        "verdict": 0.0,
        "scene_set_appos": 0.11,
        "tautological_restate": 0.0,
        "sensory_pair": 0.0,
        "procedural_chain": 0.0,
        "felt_like_abstract": 0.0,
        "which_was_meta": 0.0,
        "but_only_because": 0.0,
        "kind_the_kind": 0.0,
        "so_aux_inversion": 0.0,
        "observer_ing": 0.0,
        "who_had_decided": 0.0,
        "triple_noun": 0.0,
        "polysyndetic": 0.0,
        "staccato": 2.85,
        "anaphoric_escalation": 0.0,
        "aphoristic_couplet": 0.0,
        "mid_appositive": 1.14,
        "triple_fragment": 1.59,
        "triple_adj_stack": 0.0,
        "anaphoric_relaxed": 0.0,
        "summative_closer": 3.08,
        "which_stack": 0.0,
        "body_inventory": 0.0,
        "participle_cascade": 1.37,
        "tense_shift": 0.34,
    },
    "sentence_length_dist": {
        ...
        "median": 15,
        "stdev": 9.0,
        "p10": 4, "p25": 8, "p75": 21, "p90": 27,
        "short_lt5_rate": 0.136,
        "long_gt30_rate": 0.059,
        "long_gt40_rate": 0.012,
    },
}
# Density gating thresholds. A pattern fails the cap if its chapter density
# exceeds DENSITY_FAIL_MULTIPLIER × corpus density. For patterns where the
# corpus density is zero, the chapter must stay below DENSITY_ZERO_FLOOR.
# Each AI-only instance is a real signal but rare incidental hits (1 per
# 2000 words) are tolerated to leave headroom for legitimate one-off uses.
DENSITY_FAIL_MULTIPLIER = 2.0
DENSITY_ZERO_FLOOR = 0.5  # per 1000 words
# Em-dash floor — invert the cap. Chapter must have at least this density
# of em-dashes per 1000 words. Below this, the chapter fails. The corpus
# baseline is 3.08/1k; the floor at half that leaves stylistic headroom.
EM_DASH_FLOOR_PER_1K = 1.5
# Sentence-length distribution gates. The chapter fails any of:
#  median below MIN_MEDIAN_RATIO × corpus median
#  very-short rate above MAX_SHORT_RATIO × corpus short rate
#  very-long rate above MAX_LONG_RATIO × corpus long rate
DIST_MIN_MEDIAN_RATIO = 0.7
DIST_MAX_SHORT_RATIO = 1.5
DIST_MAX_LONG_RATIO = 2.0
# Patterns whose v32 absolute-zero hard cap is REMOVED in v33 because
# the corpus baseline shows them at higher rate than the AI chapter.
# These are still detected and reported, but they no longer gate drafts.
V33_REMOVED_FROM_HARD_CAP = {
    "triple_fragment_inventory",
    "participle_cascade",
    "mid_appositive",
}
AI_TELL_WORDS = {
    ...
    # v32 entries (installed via _v32_install_ai_tell_entries after the v32
    # patterns and trim callables are defined later in the file).
}
# ============================================================================
...
# ============================================================================
# v32 — pattern classes targeting Originality-orange cadences scan_draft
# missed in v31. See module docstring for class summary. Six are trim-able
# via callable replacements in AI_TELL_WORDS (Classes A, B, D, E, F, G);
# three are GRAFT_ONLY (C, H, I).
# ============================================================================
# v32-A — Triple noun-fragment inventory. Sentence fragment opening with a
# capitalized noun phrase, three+ comma-separated phrases, no finite verb
# in the main clause. TRIPLE_NOUN_PATTERN already catches the EVAL_HEAD-
# anchored variant ("Wet sand, flat tide, sky..."). v32-A catches the
# noun-anchored variant ("Salt along the living room sills, the bathroom
# sill, the narrow sash above the bed.") that slips past TRIPLE_NOUN
# because "Salt" / "Knees" / etc. aren't on the EVAL_HEAD list.
# Sentence-level scan; the regex below is permissive on the sentence
# structure and the scanner does the fragment + verb-absence gate.
# Trim: drop the third phrase, join phrases 1 and 2 with " and ".
TRIPLE_FRAGMENT_FINITE_VERB = re.compile(
    r"\b(?:was|were|is|are|am|had|has|have|did|does|do|will|can|could|"
    r"would|should|might|may|shall|must|been|being|got|gets|"
    # past-tense action verbs that signal a non-fragment sentence
    r"went|came|saw|took|made|gave|put|set|kept|knew|thought|felt|"
    r"said|told|asked|laughed|smiled|nodded|shook|turned|looked|"
    r"walked|ran|sat|stood|lay|drove|drank|ate|slept|woke|opened|"
    r"closed|started|stopped|left|reached|pulled|pushed|grabbed|"
    r"held|carried|moved|stepped|crossed|entered|spoke|cried|"
    r"watched|listened|heard|smelled|tasted|kissed|touched|raised|"
    r"lifted|dropped|hit|broke|fixed|wrote|read)\b",
    re.IGNORECASE,
)
# v32-B — Three-adjective stack with two internal commas inside an article
# phrase. ", a steady, awful, patient line." MID_APPOSITIVE_PATTERN
# requires three internal commas (a 4-element appositive). This catches
# the 3-element triadic adjective stack that has only two internal
# commas. The leading anchor is word-boundary + article (not comma) —
# the article phrase can be preceded by any word ("in a steady..." is
# the canonical case). Trim: keep the article + first adjective + final
# noun; drop adjectives 2 and 3.
TRIPLE_ADJECTIVE_STACK_PATTERN = re.compile(
    r"\b((?:a|an|the)\s+)"                  # group 1: "a "
    r"([A-Za-z][\w']{1,15}?)"               # group 2: adj1
    r",\s+"
    r"([A-Za-z][\w']{1,15}?)"               # group 3: adj2
    r",\s+"
    r"([A-Za-z][\w']{1,15}?)"               # group 4: adj3
    r"(\s+[a-z]\w+(?:\s+\w+){0,2})"         # group 5: trailing noun(s)
    r"(?=[.!?;,])",
    re.IGNORECASE,
)
# v32-E — Stacked "which was/is/were/had been" relative clauses (3+ in a
# single sentence with bare predicates). v31's WHICH_WAS_META catches
# specific meta-predicates; this catches the structural stacking
# regardless of predicate. Detection: count ", which (was|is|were|had
# been)" occurrences in a sentence; flag at 3+. Trim: keep the first
# relative clause; delete subsequent ones up to the next strong
# punctuation (period or em-dash).
WHICH_RELATIVE_OCCURRENCE = re.compile(
    r",\s+which\s+(?:was|is|were|had\s+been)\b",
    re.IGNORECASE,
)
# v32-F — Hedged noun-appositive after a comma: ", a [N] [pron] [aux]
# [adv]? [verb] if [pron] [verb]..." Sibling of SCENE_SET_APPOS_MARKER_C/E
# (gerund-as-subject form). This is the noun-anchored hedged appositive.
# Wired into _scan_scene_set_appositive marker list as "scene_set_hedged".
# Trim: delete from the leading comma through end of the appositive
# (terminates at sentence-end or next strong punct).
SCENE_SET_APPOS_MARKER_F = re.compile(
    r",\s+(?:a|an|the)\s+\w+(?:\s+\w+){0,2}\s+"
    r"(?:I|she|he|they|we|you)\s+"
    r"(?:could|would|had|might|did|can|may)\s+"
    r"(?:mostly|sometimes|just|barely|hardly|never|always|almost|"
    r"only|usually|still|already)?\s*"
    r"\w+\s+if\s+(?:I|she|he|they|we|you)\b",
    re.IGNORECASE,
)
# v32-G — Body-part inventory in a single sentence. Three or more
# body-part phrases comma-separated, in absolute construction (no finite
# verb between body-noun and modifier). Distinct from
# _scan_sensory_fragment_pair, which catches two consecutive
# [The/A]-opener sentences. This pattern catches the within-sentence
# body-inventory tricolon. Trim: keep the first body-phrase; drop
# subsequent.
BODY_PART_NOUNS = (
    r"(?:[Hh]ands?|[Mm]outh|[Tt]hroat|[Nn]eck|[Bb]ack|[Cc]hest|[Ss]tomach|"
    r"[Jj]aw|[Tt]eeth|[Tt]ongue|[Kk]nees?|[Ss]houlders?|[Ff]ingers?|"
    r"[Ss]pine|[Ss]kull|[Gg]ut|[Bb]elly|[Rr]ibs|[Tt]emples?|[Ww]rists?|"
    r"[Aa]nkles?|[Hh]ead|[Ll]ips?|[Pp]alms?|[Ee]lbows?)"
)
BODY_PART_INVENTORY_PATTERN = re.compile(
    r"(?:^|(?<=[.!?\u201d\"])\s+)"
    + BODY_PART_NOUNS + r"\s+(?:[a-z]\w+(?:\s+\w+){0,4}),\s+"
    + BODY_PART_NOUNS + r"\s+(?:[a-z]\w+(?:\s+\w+){0,4}),\s+"
    + r"(?:the\s+(?:back|front|inside|side|bottom|top)\s+of\s+"
    r"(?:my|her|his|their|the)\s+)?"
    + BODY_PART_NOUNS + r"\s+(?:[a-z]\w+(?:\s+\w+){0,5})"
    r"(?=[.!?])",
    re.MULTILINE,
)
# v32-H — Lyrical participial cascade. Long sentence (≥25 words) with
# 3+ participial phrases (-ing or -ed verb tokens not governed by an
# auxiliary) AND ≤1 finite main-clause verb. GRAFT_ONLY — deletion
# can't safely repair a participial cascade.
LYRICAL_PARTICIPLE_TOKEN = re.compile(
    r"\b\w+(?:ing|ed)\b",
    re.IGNORECASE,
)
LYRICAL_PARTICIPLE_AUX = re.compile(
    r"\b(?:was|were|is|are|am|had|has|have|been|being|got|getting)\s+\w+(?:ing|ed)\b",
    re.IGNORECASE,
)
LYRICAL_PARTICIPLE_FINITE_VERB = re.compile(
    r"\b(?:was|were|is|are|am|had|has|have|did|does|do|will|can|could|"
    r"would|should|might|may|came|went|saw|took|made|gave|put|set|kept|"
    r"told|said|knew|felt|got|gets|sat|stood|lay|walked|ran|drove)\b",
    re.IGNORECASE,
)
# v32-I — Tense-shift event. Per-sentence detector that fires when a
# non-dialogue sentence's tense conflicts with the chapter's dominant
# tense. GRAFT_ONLY — repair requires a same-tense same-beat graft.
TENSE_PRESENT_AUX = re.compile(
    r"\b(?:is|are|am|do|does|don't|doesn't|has|have|hasn't|haven't|"
    r"says|tells|asks|walks|runs|sits|stands|knows|feels|thinks|wants|"
    r"needs|comes|goes|sees|takes|gives|makes|gets|holds|opens|closes)\b",
    re.IGNORECASE,
)
TENSE_PAST_AUX = re.compile(
    r"\b(?:was|were|had|did|didn't|hadn't|wasn't|weren't|"
    r"said|told|asked|walked|ran|sat|stood|knew|felt|thought|wanted|"
    r"needed|came|went|saw|took|gave|made|got|held|opened|closed|"
    r"\w+ed)\b",
    re.IGNORECASE,
)
...
def _scan_scene_set_appositive(sentence_spans: list, text: str) -> list:
    ...
    # broader treatment.
    COMMA_REQUIRED = {"scene_set_which_was", "scene_set_and_was", "scene_set_because_meta", "scene_set_hedged"}
    ...
    for span_start, span_end, sent_text in sentence_spans:
        ...
        for label, pat in [
            ...
            ("scene_set_hedged", SCENE_SET_APPOS_MARKER_F),
        ]:
...
def _scan_procedural_chain(sentence_spans: list, text: str) -> list:
    ...
    return violations
# ============================================================================
# v32 scanner functions — see pattern definitions block for class summary.
# ============================================================================
def _scan_triple_fragment_inventory(sentence_spans: list, text: str) -> list:
    """v32-A — Triple noun-fragment inventory (Salt/Knees-style)."""
    violations = []
    for span_start, span_end, sent_text in sentence_spans:
        if _is_dialogue_sentence(sent_text, text[:span_start]):
            continue
        wc = _word_count(sent_text)
        if wc < 6 or wc > 30:
            continue
        # Must have at least 2 internal commas (3+ phrases)
        comma_count = sent_text.count(",")
        if comma_count < 2:
            continue
        # Skip if TRIPLE_NOUN_PATTERN already catches it (eval-head variant)
        if TRIPLE_NOUN_PATTERN.search(sent_text):
            continue
        # Fragment gate: no finite verb in the sentence (excluding dialogue
        # and quoted-thought spans). This is the structural fingerprint.
        if TRIPLE_FRAGMENT_FINITE_VERB.search(sent_text):
            continue
        # Sentence must open with a capitalized word — the "noun-anchored"
        # part of the pattern. Strip leading dialogue glyphs first.
        cleaned = sent_text.lstrip().lstrip('"\u201c\u201d').lstrip()
        if not cleaned or not cleaned[:1].isupper():
            continue
        violations.append({
            "rule": "triple_fragment_inventory",
            "context": sent_text.replace("\n", " ").strip()[:240],
            "verbatim": sent_text,
        })
    return violations
def _scan_triple_adjective_stack(text: str, sentence_spans: list) -> list:
    """v32-B — Three-adjective stack (", a steady, awful, patient line.")"""
    violations = []
    for m in TRIPLE_ADJECTIVE_STACK_PATTERN.finditer(text):
        # Skip dialogue
        before = text[: m.start()]
        normalized = before.replace("\u201c", '"').replace("\u201d", '"')
        if normalized.count('"') % 2 == 1:
            continue
        # Require at least 2 of 3 adjective slots to be EVAL_HEAD-like
        adjs = [m.group(2), m.group(3), m.group(4)]
        eval_count = sum(1 for a in adjs if _EVAL_HEAD_RE.match(a))
        if eval_count < 2:
            continue
        ctx_start = max(0, m.start() - 30)
        ctx_end = min(len(text), m.end() + 20)
        # Find verbatim sentence
        verbatim = ""
        for s, e, t in sentence_spans:
            if s <= m.start() < e:
                verbatim = t
                break
        violations.append({
            "rule": "triple_adjective_stack",
            "context": text[ctx_start:ctx_end].replace("\n", " ").strip(),
            "verbatim": verbatim or text[ctx_start:ctx_end].strip(),
        })
    return violations
def _scan_anaphoric_relaxed(sentence_spans: list, text: str) -> list:
    """v32-C — Relaxed anaphora detector. Two added tiers beyond v27's
    3-in-5-with-2-word-opener. Tier 2: 2 sentences sharing a 3+ word
    opener within a 3-sentence window. Tier 3: 3 sentences sharing
    1-word opener (with same first content word) AND a shared 2-word
    opener for at least 2 of the 3."""
    n = len(sentence_spans)
    if n < 2:
        return []
    # Compute openers (up to first 3 content words, lowercased)
    openers = []
    for (_, _, s) in sentence_spans:
        cleaned = s.lstrip().lstrip('"\u201c\u201d').lstrip()
        words = re.findall(r"\b[\w']+\b", cleaned)
        openers.append(tuple(w.lower() for w in words[:3]))
    violations = []
    seen = set()
    # Tier 2: 2 consecutive non-dialogue sentences with same 3-word opener
    for i in range(n - 1):
        a_open = openers[i]
        b_open = openers[i + 1]
        if len(a_open) < 3 or len(b_open) < 3:
            continue
        ...
        seen.add(sig)
        violations.append({
            "rule": "anaphoric_relaxed",
            "form": "tier2_3word_pair",
            "context": (a_text + " | " + b_text).replace("\n", " ").strip()[:240],
            "verbatim": a_text + " " + b_text,
        })
    # Tier 3: 3+ sentences in a 5-window sharing 1-word opener AND at least 2
    # of them sharing 2-word opener. Catches "She walked past X. She walked
    # past Y. She kept walking..." which v27 anaphora misses (broken third).
    for i in range(n):
        end = min(i + 5, n)
        window = openers[i:end]
        from collections import Counter
        c1 = Counter(o[:1] for o in window if o)
        for key1, cnt1 in c1.items():
            if cnt1 < 3:
                continue
            indices_1 = [j for j in range(i, end) if openers[j][:1] == key1]
            # Among those, count 2-word matches
            c2 = Counter(openers[j][:2] for j in indices_1 if len(openers[j]) >= 2)
            best_2word_count = max(c2.values()) if c2 else 0
            if best_2word_count < 2:
                continue
            # Skip if all are dialogue
            non_dialogue = [
                j for j in indices_1
                if not _is_dialogue_sentence(
                    sentence_spans[j][2], text[:sentence_spans[j][0]]
                )
            ]
            if len(non_dialogue) < 3:
                continue
            sig = ("tier3", tuple(non_dialogue))
            if sig in seen:
                continue
            seen.add(sig)
            run_text = " | ".join(
                sentence_spans[j][2][:60] for j in non_dialogue[:3]
            )
            violations.append({
                "rule": "anaphoric_relaxed",
                "form": "tier3_broken_third",
                "context": f"opener='{key1[0]}': {run_text}",
                "verbatim": " ".join(
                    sentence_spans[j][2] for j in non_dialogue
                ),
            })
    return violations
def _is_summative_short(sent_text: str) -> bool:
    """Heuristic for v32-D. A short sentence is summative if it lacks an
    action-progressive verb and matches one of the summative shapes:
    noun-fragment, copula+abstract, "Plain X" / "Pure X" / "Just X",
    bare-time/date statement, or naked-state declaration."""
    s = sent_text.strip().rstrip('.!?').strip()
    if not s:
        return False
    # Action-progressive verbs disqualify (catches "I went to the door.")
    if re.search(
        r"\b(?:went|came|walked|ran|drove|sat|stood|reached|opened|"
        r"closed|grabbed|pulled|pushed|lifted|dropped|carried|moved|"
        r"started|stopped|picked|set|put|took|left|entered|crossed|"
        r"called|shouted|whispered|cried|laughed|nodded|shook|"
        r"turned|stepped|knelt|leaned)\b",
        s, re.IGNORECASE,
    ):
        return False
    # Shape 1: bare noun-fragment (no finite verb at all)
    if not TRIPLE_FRAGMENT_FINITE_VERB.search(s):
        return True
    # Shape 2: "Plain X" / "Pure X" / "Just X" closer
    if re.match(r"^(?:Plain|Pure|Just|Simple|Only|Merely|Truly)\b", s, re.IGNORECASE):
        return True
    # Shape 3: clock/date statement
    if re.search(r"\b\d{1,2}:\d{2}\b", s) or re.match(
        r"^(?:The\s+)?(?:clock|hour|time|day|date)\b", s, re.IGNORECASE
    ):
        return True
    # Shape 4: naked-state declaration ("X was Y" with abstract Y / no progress)
    if re.match(
        r"^(?:[A-Z]\w*\s+){0,4}(?:was|were|is|are)\s+"
        r"(?:holding|gone|over|done|dark|quiet|still|empty|silent|broken|"
        r"closed|shut|true|real|enough|fine|right|wrong|too|so|just|"
        r"the\s+\w+|a\s+\w+\s+\w+|whole|alone|cold|warm)\b",
        s, re.IGNORECASE,
    ):
        return True
    return False
def _scan_summative_short_closer(sentence_spans: list, text: str) -> list:
    """v32-D — Summative short closer following a longer sentence."""
    violations = []
    n = len(sentence_spans)
    for i in range(n - 1):
        a_text = sentence_spans[i][2]
        ...
        a_wc = _word_count(a_text)
        b_wc = _word_count(b_text)
        if a_wc < 15 or b_wc > 10 or b_wc < 1:
            continue
        if _is_dialogue_sentence(a_text, text[:sentence_spans[i][0]]):
            continue
        if _is_dialogue_sentence(b_text, text[:sentence_spans[i + 1][0]]):
            continue
        if not _is_summative_short(b_text):
            continue
        violations.append({
            "rule": "summative_closer",
            "context": (a_text + " // " + b_text).replace("\n", " ").strip()[:280],
            "verbatim": a_text + " " + b_text,
        })
    return violations
def _scan_which_stack(sentence_spans: list, text: str) -> list:
    """v32-E — Stacked which-was relative clauses (3+ in one sentence)."""
    violations = []
    for span_start, span_end, sent_text in sentence_spans:
        if _is_dialogue_sentence(sent_text, text[:span_start]):
            continue
        occurrences = list(WHICH_RELATIVE_OCCURRENCE.finditer(sent_text))
    ...
    return violations
def _scan_body_inventory(text: str, sentence_spans: list) -> list:
    """v32-G — Body-part inventory in a single sentence (3+ body phrases)."""
    violations = []
    for m in BODY_PART_INVENTORY_PATTERN.finditer(text):
        before = text[: m.start()]
        normalized = before.replace("\u201c", '"').replace("\u201d", '"')
        if normalized.count('"') % 2 == 1:
            continue
        # Find verbatim sentence containing the match
        verbatim = ""
        for s, e, t in sentence_spans:
            if s <= m.start() < e:
                verbatim = t
        ...
        violations.append({
            "rule": "body_inventory",
            "context": text[max(0, m.start() - 20):min(len(text), m.end() + 10)].replace("\n", " ").strip(),
            "verbatim": verbatim or m.group(0),
        })
    return violations
def _scan_participle_cascade(sentence_spans: list, text: str) -> list:
    """v32-H — Long sentence with 3+ free participles and ≤1 finite verb."""
    violations = []
    for span_start, span_end, sent_text in sentence_spans:
        if _is_dialogue_sentence(sent_text, text[:span_start]):
            continue
        ...
        if wc < 25:
            continue
        # Count all -ing/-ed tokens
        all_participles = LYRICAL_PARTICIPLE_TOKEN.findall(sent_text)
        # Subtract aux+participle pairs (those are governed, not free)
        aux_pairs = LYRICAL_PARTICIPLE_AUX.findall(sent_text)
        # Each aux_pair contains one participle, so subtract len(aux_pairs)
        free_count = len(all_participles) - len(aux_pairs)
        if free_count < 3:
            continue
        # Count finite verbs (must be ≤1 for cascade designation)
        finite_count = len(LYRICAL_PARTICIPLE_FINITE_VERB.findall(sent_text))
        if finite_count > 1:
            continue
        violations.append({
            "rule": "participle_cascade",
            "context": sent_text.replace("\n", " ").strip()[:280],
            "verbatim": sent_text,
        })
    return violations
def _classify_sentence_tense(sent_text: str) -> str:
    """Return 'past', 'present', or 'ambiguous'. Used by tense-shift scan.
    Naive heuristic — counts past-aux and present-aux tokens, returns the
    majority. Ties or low counts return 'ambiguous'."""
    past_hits = len(TENSE_PAST_AUX.findall(sent_text))
    present_hits = len(TENSE_PRESENT_AUX.findall(sent_text))
    if past_hits == 0 and present_hits == 0:
        return "ambiguous"
    if past_hits > present_hits:
        return "past"
    if present_hits > past_hits:
        return "present"
    return "ambiguous"
def _scan_tense_shift(sentence_spans: list, text: str) -> list:
    """v32-I — Sentences whose tense conflicts with the chapter dominant.
    Two-pass: classify each sentence, identify dominant tense, flag
    minority-tense non-dialogue non-ambiguous sentences."""
    if len(sentence_spans) < 6:
        return []
    classifications = []
    for span_start, _, sent_text in sentence_spans:
        if _is_dialogue_sentence(sent_text, text[:span_start]):
            classifications.append("dialogue")
            continue
        if _word_count(sent_text) < 5:
            classifications.append("short")
            continue
        classifications.append(_classify_sentence_tense(sent_text))
    past_count = classifications.count("past")
    present_count = classifications.count("present")
    if past_count + present_count < 6:
        return []
    if abs(past_count - present_count) < 4:
        # No clear dominant tense; don't flag anything
        return []
    dominant = "past" if past_count > present_count else "present"
    minority = "present" if dominant == "past" else "past"
    violations = []
    for i, cls in enumerate(classifications):
        if cls != minority:
            continue
        sent_text = sentence_spans[i][2]
        violations.append({
            "rule": "tense_shift",
            "form": f"minority_{minority}_in_{dominant}",
            "context": sent_text.replace("\n", " ").strip()[:240],
            "verbatim": sent_text,
        })
    return violations
# ============================================================================
# v32 trim functions — callable replacements for AI_TELL_WORDS entries.
# Each accepts a re.Match and returns the trimmed string. The callable
# fits into apply_deletion_heuristic via re.sub's native support for
# function replacements.
# ============================================================================
def _trim_triple_adjective_stack(m) -> str:
    """v32-B trim: keep article + adj1 + trailing noun; drop adj2, adj3."""
    article_segment = m.group(1)        # ", a "
    adj1 = m.group(2)
    trailing = m.group(5)               # " line"
    return f"{article_segment}{adj1}{trailing}"
def _trim_which_stack(m) -> str:
    """v32-E trim: keep the first ', which (was|is|...)' clause; drop
    subsequent clauses. The match starts at the first comma; we cut at
    ...
    if not second:
        return full
    return full[: 1 + second.start()]
def _trim_sentence_triple_fragment(sentence: str) -> str:
    """Trim Class A: 'X, Y, Z.' → 'X and Y.'  Drop the third NP, join 1+2 with 'and'."""
    # Simple regex: capture last two comma-separated phrases before sentence-end.
    pattern = re.compile(r",\s+([^,]+?),\s+([^,.!?]+)([.!?])\s*$")
    m = pattern.search(sentence)
    if not m:
        return sentence
    second_phrase = m.group(1).strip()
    end_punct = m.group(3)
    return sentence[:m.start()] + " and " + second_phrase + end_punct
def _trim_sentence_body_inventory(sentence: str) -> str:
    """Trim Class G: keep the first body-phrase, drop subsequent."""
    m = BODY_PART_INVENTORY_PATTERN.search(sentence)
    if not m:
        return sentence
    # The match spans three body-phrases. The first body-phrase ends at
    # the first comma inside the match.
    matched_text = m.group(0)
    first_comma = matched_text.find(",")
    if first_comma < 0:
        return sentence
    first_phrase = matched_text[:first_comma]
    # Replace the entire match with first_phrase + sentence-end punct.
    end_punct = matched_text[-1] if matched_text[-1:] in ".!?" else "."
    replacement = first_phrase + end_punct
    return sentence[:m.start()] + replacement + sentence[m.end():]
def _trim_sentence_hedged_appositive(sentence: str) -> str:
    """Trim Class F: delete the comma-prefixed hedged appositive."""
    m = SCENE_SET_APPOS_MARKER_F.search(sentence)
    if not m:
        return sentence
    # Cut from the leading comma to the next strong punctuation
    # (period/em-dash/semicolon) or sentence-end.
    cut_start = m.start()
    # Find next strong punct after match end
    rest = sentence[m.end():]
    next_strong = re.search(r"[.!?;\u2014]", rest)
    if next_strong:
        cut_end = m.end() + next_strong.start()
        # Keep the strong punct on the prior clause
        result = sentence[:cut_start] + sentence[cut_end:]
    else:
        # Goes to end of sentence; replace with end-punct
        end_punct = sentence[-1] if sentence and sentence[-1] in ".!?" else "."
        result = sentence[:cut_start] + end_punct
    # Collapse double spaces
    result = re.sub(r"  +", " ", result)
    return result
def _trim_summative_closer_pair(text: str, verbatim: str) -> str:
    """Trim Class D: delete the second sentence (the closer) of the pair.
    Returns the modified `text` with the closer removed."""
    if verbatim not in text:
        return text
    # Split the verbatim into the long sentence and the closer.
    # verbatim is "<long sent> <short closer>" with a space joining them.
    # Use the same sentence splitter the scanner uses.
    parts = _split_sentences_with_offsets(verbatim)
    if len(parts) < 2:
        return text
    long_sent = parts[0][2]
    return text.replace(verbatim, long_sent, 1)
def _trim_hedged_appositive_match(m) -> str:
    """v32-F trim called from apply_deletion_heuristic. Returns empty
    string — the leading comma + appositive clause is deleted in place.
    apply_deletion_heuristic's whitespace-collapse step cleans up."""
    return ""
def _v32_install_ai_tell_entries():
    """Install v32 entries into AI_TELL_WORDS. Trim-able classes get
    callable replacements; GRAFT_ONLY classes get None."""
    # Class B — three-adjective stack (trim)
    AI_TELL_WORDS["v32_triple_adjective_stack"] = [
        (TRIPLE_ADJECTIVE_STACK_PATTERN, _trim_triple_adjective_stack),
    ]
    # Class C — relaxed anaphora (graft-only). The marker is permissive;
    # actual flagging happens via _scan_anaphoric_relaxed in scan_draft.
    AI_TELL_WORDS["v32_anaphoric_relaxed"] = [
        (
            re.compile(
                r"^\s*[\"\u201c]?(?:She|He|I|They|We|It)\s+"
                r"(?:does\s+not|did\s+not|walked|walks|kept|keeps|"
                r"came|comes|went|goes|stood|stands|watched|watches|"
                r"listened|listens)\b",
                re.IGNORECASE,
            ),
            GRAFT_ONLY,
        ),
    ]
    # Class E — stacked which-was (trim: keep first; drop subsequent)
    AI_TELL_WORDS["v32_which_stack"] = [
        (
            re.compile(
                r",\s+which\s+(?:was|is|were|had\s+been)\b[^.!?]*?,\s+"
                r"which\s+(?:was|is|were|had\s+been)\b[^.!?]*",
                re.IGNORECASE,
            ),
            _trim_which_stack,
        ),
    ]
    # Class F — hedged noun-appositive (trim: delete the appositive)
    AI_TELL_WORDS["v32_hedged_appositive"] = [
        (
            re.compile(
                r",\s+(?:a|an|the)\s+\w+(?:\s+\w+){0,2}\s+"
                r"(?:I|she|he|they|we|you)\s+"
                r"(?:could|would|had|might|did|can|may)\s+"
                r"(?:mostly|sometimes|just|barely|hardly|never|always|"
                r"almost|only|usually|still|already)?\s*"
                r"\w+\s+if\s+(?:I|she|he|they|we|you)\b"
                r"[^.!?;\u2014]*"        # consume through end of clause
                r"(?=[.!?;\u2014]|$)",   # stop before strong punct (don't eat it)
                re.IGNORECASE,
            ),
            _trim_hedged_appositive_match,
        ),
    ]
    # Class H — participial cascade (graft-only)
    AI_TELL_WORDS["v32_participle_cascade"] = [
        (
            re.compile(
                r"\b\w+ing\b[^.!?]{0,80}\b\w+ing\b[^.!?]{0,80}\b\w+ing\b",
                re.IGNORECASE,
            ),
            GRAFT_ONLY,
        ),
    ]
    # Class I — tense shift (graft-only)
    AI_TELL_WORDS["v32_tense_shift"] = [
        (
            re.compile(
                r"\b(?:does\s+not|doesn't|says|tells|asks|walks|runs|sits|"
                r"stands|knows|feels|thinks|wants|needs|comes|goes|sees|"
                r"takes|gives|makes|gets)\b",
                re.IGNORECASE,
            ),
            GRAFT_ONLY,
        ),
    ]
# Install v32 entries now that all patterns and callables are defined.
_v32_install_ai_tell_entries()
# ============================================================================
# v33 — Density and distribution checks against the human reference corpus.
# Each function returns a list of violation dicts (empty if no violations).
# ============================================================================
def _compute_v33_density_violations(scan_counts: dict, word_count: int,
                                     baseline: dict) -> list:
    """For each pattern in the baseline, compute chapter density per 1000
    words and compare to corpus density. Return a list of violations where
    the chapter exceeds the corpus by DENSITY_FAIL_MULTIPLIER (or by
    DENSITY_ZERO_FLOOR for patterns with corpus density zero).
    Patterns in V33_REMOVED_FROM_HARD_CAP are computed and reported but
    not flagged as violations."""
    if word_count <= 0:
        return []
    violations = []
    pattern_to_count_key = {
        "the_way": "scan_the_way_count",
        "periphrastic": "scan_periphrastic_count",
        "not_but": "scan_not_but_count",
        "em_dash": "scan_em_dash_count",
        "emotion_naming": "scan_emotion_naming_count",
        "aphoristic": "scan_aphoristic_count",
        "backfill": "scan_backfill_count",
        "verdict": "scan_verdict_count",
        "scene_set_appos": "scan_scene_set_appos_count",
        "tautological_restate": "scan_tautological_restate_count",
        "sensory_pair": "scan_sensory_pair_count",
        "procedural_chain": "scan_procedural_chain_count",
        "felt_like_abstract": "scan_felt_like_abstract_count",
        "which_was_meta": "scan_which_was_meta_count",
        "but_only_because": "scan_but_only_because_count",
        "kind_the_kind": "scan_kind_the_kind_count",
        "so_aux_inversion": "scan_so_aux_inversion_count",
        "observer_ing": "scan_observer_ing_count",
        "who_had_decided": "scan_who_had_decided_count",
        "triple_noun": "scan_triple_noun_count",
        "polysyndetic": "scan_polysyndetic_count",
        "staccato": "scan_staccato_count",
        "anaphoric_escalation": "scan_anaphoric_escalation_count",
        "aphoristic_couplet": "scan_aphoristic_couplet_count",
        "mid_appositive": "scan_mid_appositive_count",
        "triple_fragment": "scan_triple_fragment_count",
        "triple_adj_stack": "scan_triple_adj_stack_count",
        "anaphoric_relaxed": "scan_anaphoric_relaxed_count",
        "summative_closer": "scan_summative_closer_count",
        "which_stack": "scan_which_stack_count",
        "body_inventory": "scan_body_inventory_count",
        "participle_cascade": "scan_participle_cascade_count",
        "tense_shift": "scan_tense_shift_count",
    }
    # Patterns whose internal name differs from the rule string used in
    # violations. V33_REMOVED_FROM_HARD_CAP uses the rule string form.
    name_aliases = {
        "triple_fragment": "triple_fragment_inventory",
    }
    densities = baseline.get("densities_per_1k", {})
    for pattern, count_key in pattern_to_count_key.items():
        chapter_count = scan_counts.get(count_key, 0) or 0
        chapter_density = round(1000 * chapter_count / word_count, 2)
        corpus_density = densities.get(pattern, 0.0)
        rule_name = name_aliases.get(pattern, pattern)
        # Don't flag patterns v33 removed from hard-cap (still computed
        # and visible, just not violations)
        if rule_name in V33_REMOVED_FROM_HARD_CAP:
            continue
        # Em-dash gets a floor check, not a density-fail check
        if pattern == "em_dash":
            continue
        if corpus_density > 0:
            threshold = round(corpus_density * DENSITY_FAIL_MULTIPLIER, 2)
            exceeds = chapter_density > threshold
        else:
            threshold = DENSITY_ZERO_FLOOR
            exceeds = chapter_density > threshold
        if exceeds:
            violations.append({
                "pattern": pattern,
                ...
                "chapter_per_1k": chapter_density,
                "corpus_per_1k": corpus_density,
                "threshold_per_1k": threshold,
                "ratio": (
                    round(chapter_density / corpus_density, 1)
                    if corpus_density > 0 else None
                ),
            })
    return violations
def _compute_v33_em_dash_floor_violation(scan_counts: dict, word_count: int):
    """v33 em-dash floor — chapter must have at least EM_DASH_FLOOR_PER_1K
    em-dashes per 1000 words. The corpus baseline is 3.08/1k; the floor
    at half that leaves stylistic headroom but prevents the AI from
    suppressing em-dashes entirely. Returns the violation dict, or None."""
    if word_count <= 0:
        return None
    em_count = scan_counts.get("scan_em_dash_count", 0) or 0
    em_density = round(1000 * em_count / word_count, 2)
    if em_density >= EM_DASH_FLOOR_PER_1K:
        return None
    return {
        "rule": "em_dash_floor",
        "chapter_count": em_count,
        "chapter_per_1k": em_density,
        "floor_per_1k": EM_DASH_FLOOR_PER_1K,
        "corpus_per_1k": CORPUS_BASELINE_V33["densities_per_1k"]["em_dash"],
    }
def _compute_v33_distribution_metrics(text: str) -> dict:
    """Compute sentence-length distribution metrics for the chapter.
    Mean, median, stdev, very-short rate, very-long rate."""
    ...
    mean_v = sum(lengths) / n
    median_v = sorted_lengths[n // 2]
    if n > 1:
        var = sum((x - mean_v) ** 2 for x in lengths) / (n - 1)
        stdev_v = var ** 0.5
    else:
        stdev_v = 0.0
    short_count = sum(1 for x in lengths if x < 5)
    long30_count = sum(1 for x in lengths if x > 30)
    long40_count = sum(1 for x in lengths if x > 40)
    return {
        "sentence_count": n,
        "mean": round(mean_v, 1),
        "median": median_v,
        "stdev": round(stdev_v, 1),
        "min": min(lengths),
        "max": max(lengths),
        "short_lt5_rate": round(short_count / n, 3),
        "long_gt30_rate": round(long30_count / n, 3),
        "long_gt40_rate": round(long40_count / n, 3),
    }
def _compute_v33_distribution_violations(metrics: dict, baseline: dict) -> list:
    """Compare chapter distribution metrics to corpus baseline. Return
    list of violations where the chapter shape diverges from human."""
    if not metrics:
        return []
    violations = []
    corpus = baseline.get("sentence_length_dist", {})
    corpus_median = corpus.get("median", 15)
    corpus_short = corpus.get("short_lt5_rate", 0.14)
    corpus_long = corpus.get("long_gt30_rate", 0.06)
    chapter_median = metrics.get("median", 0)
    median_ratio = chapter_median / corpus_median if corpus_median else 1.0
    if median_ratio < DIST_MIN_MEDIAN_RATIO:
        violations.append({
            "rule": "sentence_median_too_low",
            "chapter_median": chapter_median,
            "corpus_median": corpus_median,
            "ratio": round(median_ratio, 2),
            "min_ratio": DIST_MIN_MEDIAN_RATIO,
        })
    chapter_short = metrics.get("short_lt5_rate", 0.0)
    short_ratio = chapter_short / corpus_short if corpus_short else 1.0
    if short_ratio > DIST_MAX_SHORT_RATIO:
        violations.append({
            "rule": "very_short_rate_too_high",
            "chapter_short_rate": chapter_short,
            "corpus_short_rate": corpus_short,
            "ratio": round(short_ratio, 2),
            "max_ratio": DIST_MAX_SHORT_RATIO,
        })
    chapter_long = metrics.get("long_gt30_rate", 0.0)
    long_ratio = chapter_long / corpus_long if corpus_long else 1.0
    if long_ratio > DIST_MAX_LONG_RATIO:
        violations.append({
            "rule": "very_long_rate_too_high",
            "chapter_long_rate": chapter_long,
            "corpus_long_rate": corpus_long,
            "ratio": round(long_ratio, 2),
            "max_ratio": DIST_MAX_LONG_RATIO,
        })
    return violations
def scan_draft(text: str) -> dict:
    ...
    # ----- v32 scanners -----
    triple_fragment_violations = _scan_triple_fragment_inventory(
        _v27_sentence_spans, text,
    )
    flagged.extend(triple_fragment_violations[:15])
    triple_adj_stack_violations = _scan_triple_adjective_stack(
        text, _v27_sentence_spans,
    )
    flagged.extend(triple_adj_stack_violations[:15])
    anaphoric_relaxed_violations = _scan_anaphoric_relaxed(
        _v27_sentence_spans, text,
    )
    flagged.extend(anaphoric_relaxed_violations[:15])
    summative_closer_violations = _scan_summative_short_closer(
        _v27_sentence_spans, text,
    )
    flagged.extend(summative_closer_violations[:15])
    which_stack_violations = _scan_which_stack(
        _v27_sentence_spans, text,
    )
    flagged.extend(which_stack_violations[:15])
    body_inventory_violations = _scan_body_inventory(
        text, _v27_sentence_spans,
    )
    flagged.extend(body_inventory_violations[:15])
    participle_cascade_violations = _scan_participle_cascade(
        _v27_sentence_spans, text,
    )
    flagged.extend(participle_cascade_violations[:15])
    tense_shift_violations = _scan_tense_shift(
        _v27_sentence_spans, text,
    )
    flagged.extend(tense_shift_violations[:15])
    # Punctuation
    ...
    # Hard cap pass
    # ----- v33 density and distribution gating -----
    # Build the partial scan dict that the density check needs (the same
    # scan_*_count keys that go into the return dict). Density check then
    # compares chapter density to corpus baseline. Em-dash floor and
    # sentence-length distribution are also computed here.
    _v33_scan_counts = {
        ...
        "scan_triple_fragment_count": len(triple_fragment_violations),
        "scan_triple_adj_stack_count": len(triple_adj_stack_violations),
        "scan_anaphoric_relaxed_count": len(anaphoric_relaxed_violations),
        "scan_summative_closer_count": len(summative_closer_violations),
        "scan_which_stack_count": len(which_stack_violations),
        "scan_body_inventory_count": len(body_inventory_violations),
        "scan_participle_cascade_count": len(participle_cascade_violations),
        "scan_tense_shift_count": len(tense_shift_violations),
    }
    v33_density_violations = _compute_v33_density_violations(
        _v33_scan_counts, wc, CORPUS_BASELINE_V33,
    )
    v33_em_dash_floor_violation = _compute_v33_em_dash_floor_violation(
        _v33_scan_counts, wc,
    )
    v33_distribution_metrics = _compute_v33_distribution_metrics(text)
    v33_distribution_violations = _compute_v33_distribution_violations(
        v33_distribution_metrics, CORPUS_BASELINE_V33,
    )
    # v33 hard cap: pass iff every gate passes. The structural patterns
    # in V33_REMOVED_FROM_HARD_CAP are excluded from the density check
    # already (see _compute_v33_density_violations).
    hard_cap_pass = (
        len(v33_density_violations) == 0
        and v33_em_dash_floor_violation is None
        and len(v33_distribution_violations) == 0
    ...
    return {
        ...
        "scan_participle_cascade_count": len(participle_cascade_violations),
        "scan_tense_shift_count": len(tense_shift_violations),
        # v33 density and distribution metrics
        "scan_v33_density_violations": v33_density_violations,
        "scan_v33_em_dash_floor_violation": v33_em_dash_floor_violation,
        "scan_v33_distribution_metrics": v33_distribution_metrics,
        "scan_v33_distribution_violations": v33_distribution_violations,
        ...
        "scan_flagged_passages": json.dumps(flagged[:120], ensure_ascii=False),
    }
def format_scan_summary(scan: dict) -> str:
    ...
    # v32 summary entries
    if scan.get("scan_triple_fragment_count", 0):
        flags.append(f"tri-frag×{scan['scan_triple_fragment_count']}")
    if scan.get("scan_triple_adj_stack_count", 0):
        flags.append(f"tri-adj×{scan['scan_triple_adj_stack_count']}")
    if scan.get("scan_anaphoric_relaxed_count", 0):
        flags.append(f"anaphora-relax×{scan['scan_anaphoric_relaxed_count']}")
    if scan.get("scan_summative_closer_count", 0):
        flags.append(f"summ-closer×{scan['scan_summative_closer_count']}")
    if scan.get("scan_which_stack_count", 0):
        flags.append(f"which-stack×{scan['scan_which_stack_count']}")
    if scan.get("scan_body_inventory_count", 0):
        flags.append(f"body-inv×{scan['scan_body_inventory_count']}")
    if scan.get("scan_participle_cascade_count", 0):
        flags.append(f"part-cascade×{scan['scan_participle_cascade_count']}")
    if scan.get("scan_tense_shift_count", 0):
        flags.append(f"tense-shift×{scan['scan_tense_shift_count']}")
    # v33 — density and distribution gates
    for v in scan.get("scan_v33_density_violations", []) or []:
        flags.append(
            f"density:{v['rule']}×{v['chapter_count']}"
            f"({v['chapter_per_1k']}/1k vs {v['threshold_per_1k']})"
        )
    em_floor = scan.get("scan_v33_em_dash_floor_violation")
    if em_floor:
        flags.append(
            f"em-dash-floor({em_floor['chapter_per_1k']}/1k < "
            f"{em_floor['floor_per_1k']})"
        )
    for v in scan.get("scan_v33_distribution_violations", []) or []:
        flags.append(f"dist:{v['rule']}(ratio={v.get('ratio','?')})")
    ...
    for sent_text, start, end in spans:
        for word, patterns in ai_tell_words.items():
            ...
            for pat, _repl in patterns:
                # v32: support compiled patterns (their own flags) and
                # string patterns (apply IGNORECASE here).
    ...
    for word, patterns in ai_tell_words.items():
        for pat, repl in patterns:
            if repl is None:
                ...
                continue
            # v32: support compiled patterns (their own flags) and string
            # patterns (apply IGNORECASE here). repl can be a string or a
            # callable — re.sub and pattern.sub both accept callables.
            if hasattr(pat, "sub"):
            ...
            if new_result != result:
                edits.append({
                    "word": word,
                    "pattern": pat if isinstance(pat, str) else pat.pattern,
                    "before": result,
                    "after": new_result,
                })
                result = new_result
...
SYSTEM_PROMPT = (
    ...
    "DRAFTING DISCIPLINE — screen each sentence against the rules below "
    "before writing it. These rules address AI-cadence patterns that "
    "current detector tools flag in commercial fiction registers.\n\n"
    "1. EM-DASH FLOOR. Use em-dashes at approximately 3 per 1,000 words. "
    "Floor: 1.5 per 1,000 words (a 2,500-word chapter requires at least 4). "
    "Ceiling: 5 per 1,000 words. Use em-dashes for parenthetical insertions "
    "set off by a paired em-dash, dramatic interruption of a thought, and "
    "trailing emphasis at sentence end. When a sentence stacks four or more "
    "short descriptive items in comma-only series, prefer paired em-dashes "
    "as the inner separator instead of all-commas. No em-dash in the first "
    "paragraph. Never two consecutive em-dash sentences.\n\n"
    "2. PASSAGE PATTERN BUDGET (anti-stacking). Detector tools score "
    "passages orange where multiple AI-cadence patterns co-occur in a "
    "short span, even when each pattern is below its individual frequency "
    "limit. The damage is from clustering. In any 60-word window, use AT "
    "MOST ONE of the following rhythmic devices: "
    "(a) anaphoric repetition of the same subject across consecutive "
    "sentences (two or more sentences sharing identical subject and "
    "matching verb-pattern); "
    "(b) parallel negation pair (two or more consecutive sentences each "
    "opening with the same subject + negation, of the form [Subject] does "
    "not [verb-A], [Subject] does not [verb-B]); "
    "(c) templated minimal-emotion declarative followed by echo (rule 4 "
    "below); "
    "(d) aphoristic verdict close (a short final-position sentence that "
    "summarizes the moment with a definite-article noun-phrase, of the "
    "form \"That was the [noun]\" or \"[noun] was the [noun]\"); "
    "(e) triadic noun-phrase list with evaluative modifiers (three or more "
    "parallel descriptive noun-phrases in series with no main verb, of "
    "the form \"[adjective] [noun], [adjective] [noun], [adjective] [noun]\"); "
    "(f) past-perfect resonance compression (rule 3 below). "
    "If you reach for a second device in the same passage, write through "
    "it differently — full sentence with subordination, asymmetric "
    "structure, or topic break.\n\n"
    "3. PAST-PERFECT LIMIT. Do not use past-perfect (\"had X-ed\") to add "
    "weight to backstory. No more than 4 past-perfect constructions per "
    "1,000 words. No chained past-perfects (two or more past-perfect verbs "
    "joined by \"and\" inside a single sentence). No ritualized "
    "construction of the form \"the day [actor] had [verbed] and "
    "[verbed] the words that [verbed].\" No \"[subject] had been "
    "[profession-or-role-or-state] for [time-span]\" as backstory "
    "shorthand. Render flashbacks in simple past, briefly, with specific "
    "sensory detail.\n\n"
    "4. TEMPLATED EMOTION ECHO. Plain emotion-naming in first person "
    "(structure: pronoun + \"was\" + single-word emotion) may stand "
    "alone, be followed by concrete action, or be followed by dialogue. "
    "It MUST NOT be followed within two sentences by an echo-modifier "
    "sentence beginning with \"Plain [echo],\" \"That was [verdict],\" "
    "\"The kind [description],\" \"It was [recasting],\" or any other "
    "structural echo of the named emotion. The two-sentence shape — "
    "plain naming + aphoristic recast — is a high-confidence "
    "detector signal even when each sentence in isolation is "
    "unremarkable.\n\n"
    "5. SHORT FRAGMENT REPETITION. Standalone two-or-three-word "
    "subject-verb sentences (structure: pronoun-or-name + simple-past-"
    "verb + period) may appear at most ONCE per chapter for any specific "
    "fragment. Different short fragments may total at most three across "
    "the chapter, in separate scenes. Never place two short subject-verb "
    "fragments within the same paragraph. Never place two within 200 "
    "words of each other.\n\n"
    "6. LONG-SENTENCE VARIANCE. Long sentences (30+ words) should appear "
    "at corpus-typical rates: 5–8% of total sentences. Build them "
    "from cumulative concrete sensory detail, subordinate clause "
    "embedding, listed items grounded in scene specifics, or extended "
    "physical action — not from figurative reach (similes, "
    "metaphors, observational periphrasis remain restricted by the "
    "outline's hard caps). Do not collapse the distribution toward short "
    "sentences when figurative routes are closed."
)
...
def _build_winner_flags_text(winner_scan: dict) -> str:
    ...
    multi_sent_rules = {
        ...
        # v32 — multi-sentence patterns the grafter must treat as one unit
        "anaphoric_relaxed",         # tier2 pair / tier3 broken-third run
        "summative_closer",          # long sentence + short closer pair
    }
    lines = []
    for f in flags:
        rule = f.get("rule", "?")
...
def _donor_sentence_is_clean(donor_sentence: str) -> bool:
    ...
    for pat in (FELT_LIKE_ABSTRACT, BUT_ONLY_BECAUSE, KIND_THE_KIND):
        for m in pat.finditer(donor_sentence):
            ...
            if normalized.count('"') % 2 == 0:
                return False
    # WHICH_WAS_META — v31; previously not in donor-rejection
    if WHICH_WAS_META.search(donor_sentence):
        return False
    if SO_AUX_INVERSION.search(donor_sentence):
        return False
    if THE_OBSERVER_ING.search(donor_sentence):
        return False
    if WHO_HAD_DECIDED.search(donor_sentence):
        return False
    # v32 — reject donors carrying any v32 cadence
    # Class A: triple noun-fragment inventory (use scanner one-sentence at a time)
    _v32_spans = [(0, len(donor_sentence), donor_sentence)]
    if _scan_triple_fragment_inventory(_v32_spans, donor_sentence):
        return False
    # Class B: three-adjective stack (regex match outside dialogue)
    for m in TRIPLE_ADJECTIVE_STACK_PATTERN.finditer(donor_sentence):
        before = donor_sentence[: m.start()]
        normalized = before.replace("\u201c", '"').replace("\u201d", '"')
        if normalized.count('"') % 2 == 0:
            adjs = [m.group(2), m.group(3), m.group(4)]
            if sum(1 for a in adjs if _EVAL_HEAD_RE.match(a)) >= 2:
                return False
    # Class E: which-stack (3+ which-clauses in this sentence)
    if len(WHICH_RELATIVE_OCCURRENCE.findall(donor_sentence)) >= 3:
        return False
    # Class F: hedged appositive (piggybacks on scene-set markers; explicit
    # check here in case the scene-set length gate excluded it)
    if SCENE_SET_APPOS_MARKER_F.search(donor_sentence):
        return False
    # Class G: body-part inventory in single sentence
    if _scan_body_inventory(donor_sentence, _v32_spans):
        return False
    # Class H: participial cascade (single-sentence scan)
    if _scan_participle_cascade(_v32_spans, donor_sentence):
        return False
    # Class C / D / I are multi-sentence or chapter-level — they cannot be
    # detected on a single donor sentence in isolation, so they are not
    # checked here. The grafter prompt's multi_sent_rules entry handles
    # the C and D cases at the run level.
...
def _donor_passage_is_clean(donor_text: str) -> bool:
    ...
    if SO_AUX_INVERSION.search(donor_text):
        return False
    # v32: passage-level multi-sentence patterns
    if _scan_anaphoric_relaxed(sentence_spans, donor_text):
        return False
    if _scan_summative_short_closer(sentence_spans, donor_text):
        return False
    return True
