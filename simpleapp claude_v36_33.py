...
fails to import, v36.27 logs the reason and falls back to v25's shipping
behavior (the literary winner ships ungated) — the regression loop is
disabled but the rest of the pipeline still produces output.
...
v36.33 retargets the evaluator and the selection chain at commercial
fiction. Five surgical changes to v36.32, no architectural rewrite:
  1. EVALUATOR_PROMPT UNACCEPTABLE criteria (Q1 floor) reframed for
     commercial genre fiction: voice-collapse-as-generic is no longer
     a disqualifier; subtext-absence in dialogue is no longer a
     disqualifier; interpretive-narration stretches are no longer
     prose-level damage. The commercial-genre disqualifiers replace
     them: stakes invisible at the scene level, romance/suspense
     channel closed in the beat that asks for it, per-beat contract
     not delivered, hook absent at the close.
  2. EVALUATOR_PROMPT QUALITY_SCORE criteria (1-10 scale) reframed
     for commercial genre fiction: propulsion, hook strength, stakes
     legibility, character-want clarity, beat delivery against genre
     expectations, dialogue function, interior accessibility. Replaces
     the literary-fiction criteria (wit, texture, interior sharpness).
  3. EVALUATOR_PROMPT WINNER instruction changed from "the one draft
     from the RANKING line you would ship on literary grounds" to
     "the one draft from the RANKING line you would ship on
     commercial grounds — the draft most likely to land with a
     genre reader and deliver the per-beat contract items with
     clean propulsion."
  4. run_final_pass moved earlier in run_pipeline — runs after the
     ACCEPTABLE filter but BEFORE pipeline_ranking is built. This
     makes commercial_run_id available for use as a tie-breaker in
     the ridge sort and as the primary fallback when the ridge can
     not run. In v36.32 final_pass ran late and the commercial pick
     was written to disk as advisory only with no downstream effect.
  5. pipeline_ranking sort and fallback updated to use the FINAL_PASS
     commercial_run_id: ridge ties (rare but possible) break in
     favor of the commercial pick; when the ridge predictor is
     unavailable, the fallback order is commercial pick → literary
     ranking → remaining retained drafts. This replaces v36.32's
     fallback that put the literary ranking first.
The internal variable names "literary_ranking" / "literary_winner_run_id"
are NOT renamed in v36.33 — they continue to mean "the EVALUATOR's
ranking line" and "the EVALUATOR's WINNER pick," which now reflects
commercial-grounds judgment after change 3. Renaming would touch a
large number of call sites and the CSV / SUMMARY paths; that is
deferred to a later pass if naming clarity becomes a maintenance
issue. The FINAL_PASS_PROMPT (commercial vs literary final pass)
is preserved unchanged — it continues to produce both picks; the
commercial pick now drives selection, the literary pick is advisory.
The Stage F ridge feature set (six features, no v36.32 scanner
counts) is also preserved unchanged in v36.33 — that wiring is a
separate, larger problem flagged for a later pass.
"""
import base64
import io
import json
...
# ship-the-literary-winner behavior (no AI-score gate, no regen loop).
try:
...
3. QUALITY FLOOR — one verdict per draft. For each draft, decide ACCEPTABLE or UNACCEPTABLE. Apply a LENIENT standard: mark a draft ACCEPTABLE unless you would be embarrassed to ship it. UNACCEPTABLE means one or more of:
   - Stakes invisible: the scene-level stakes — what the POV character wants, what is at risk, what would change if they fail — are not legible to the reader by the end of the chapter, or are buried so far in interpretive narration that a commercial-genre reader would lose the thread.
   - Genre channel closed: the chapter's romance/suspense channel — the page where the genre promise lands, the beat the outline names for the chemistry / threat / hook — is missing, perfunctory, or dampened to the point of not registering as a romantic-suspense (or stated-genre) beat.
   ...
   - Per-beat contract not delivered: the outline's per-beat required items — named props, named information exchanges, named character actions — are absent or gestured at rather than dramatized.
   - Hook absent at the close: the chapter ends without forward pull — no question raised, no threat introduced, no escalation pending. A commercial-genre chapter must leave the reader needing the next page.
   - Dialogue that doesn't function: exchanges that don't advance the plot, don't reveal character, and don't carry stakes; turns that read as exposition dumps with no scene-level purpose.
   ...
   - Prose-level damage: runs of flat summary where the outline asks for scene, abandoned subplots, characters acting out of their profiles.
   Merely being less elegant than another draft is NOT grounds for UNACCEPTABLE. A more accessible or more direct interior voice is NOT grounds for UNACCEPTABLE — commercial genre fiction tolerates and often prefers accessible interior voice over restrained literary voice. Stylistic difference is NOT grounds for UNACCEPTABLE. A draft can be ACCEPTABLE even if another draft is better at the same beats.
4. QUALITY SCORE — one integer score per draft, on a 1–10 scale, where 10 is the strongest commercial-genre prose in this batch and 1 is the weakest prose that still functions at all. Score on commercial-fiction craft only: propulsion (sentence-to-sentence forward pull), hook strength (whether each beat ends pointing into the next), stakes legibility (whether the reader knows what is at risk), character-want clarity (whether the POV character's drive is visible on the page), genre-beat delivery (whether the outline's per-beat contract items land with the specifics the outline asks for), dialogue function (whether exchanges advance plot or reveal character with stakes attached), and interior accessibility (whether the POV character's thought is legible to a commercial-genre reader without literary-fiction decoding work). Use the scale comparatively across THIS batch. If two drafts are genuinely equal in commercial-craft quality, give them the same score. UNACCEPTABLE drafts should get a score of 0.
...
WINNER: N
(the one draft from the RANKING line you would ship on commercial grounds — the draft most likely to land with a genre reader and deliver the per-beat contract items with clean propulsion, before the downstream AI ranking breaks ties)
...
def scan_draft(text: str) -> dict:
    ...
    # Hard cap pass — v36.32 adds the three new patterns to the conjunction.
    hard_cap_pass = (
...
def run_pipeline(
    ...
    # v36.33: run final_pass HERE, before pipeline_ranking is built, so that
    # commercial_run_id is available as a tie-breaker in the ridge sort and
    # as the primary fallback when the ridge is unavailable. In v36.32 this
    # call sat AFTER pipeline_ranking was already built (around line 3726),
    # which meant the commercial pick had no path into selection — it was
    # written to disk as advisory only. The crash wrap from v36.32 is
    # preserved verbatim. The acceptable_drafts argument here is retained_drafts
    # in retained_ids order (which matches the original drafts order filtered
    # to acceptable). T1/T2/T3 positional labels in the FINAL_PASS prompt
    # therefore index into retained_drafts.
    commercial_run_id_from_final_pass = ""
    literary_run_id_from_final_pass = ""
    if len(retained_drafts) >= 2:
        try:
            final_pass_early = run_final_pass(
                client, eval_model, retained_drafts,
                outline_text, batch_stub,
            )
            result["final_pass"] = final_pass_early
            commercial_run_id_from_final_pass = final_pass_early.get("commercial_run_id", "") or ""
            literary_run_id_from_final_pass = final_pass_early.get("literary_run_id", "") or ""
        except Exception as fp_exc:
            import traceback as _fp_tb
            result["final_pass"] = {
                "error": (
                    f"run_final_pass crashed under v36.33 early-position wrap: "
    ...
    if predictor.get("available"):
        ...
        # v36.33: tie-break the ridge sort in favor of the commercial pick.
        # raw_score and predicted_score are continuous so true ties are rare,
        # but when they occur (or when the predictor returns None for one
        # draft and a real score for another), the commercial pick from the
        # final_pass evaluator should win. Sort key: (raw_score desc,
        # predicted_score desc, is_commercial_pick desc).
        _commercial_pick = commercial_run_id_from_final_pass
        ai_scores.sort(
            key=lambda item: (
                ...
                1 if item.get("run_id", "") == _commercial_pick else 0,
            ),
            reverse=True,
        )
        ...
        # v36.33: ridge unavailable → use the commercial pick as the head of
        # the fallback ranking, then the literary ranking from the evaluator
        # (which after v36.33's EVALUATOR_PROMPT WINNER change also reflects
        # commercial-grounds judgment), then any remaining retained drafts.
        # In v36.32 this fallback used lit_ranking_ids first, which routed to
        # the literary-grounds WINNER under the old prompt.
        fallback_order = []
        if commercial_run_id_from_final_pass and commercial_run_id_from_final_pass in retained_ids:
            fallback_order.append(commercial_run_id_from_final_pass)
        for rid in lit_ranking_ids:
            if rid in retained_ids and rid not in fallback_order:
                fallback_order.append(rid)
        for rid in retained_ids:
            if rid not in fallback_order:
                fallback_order.append(rid)
        pipeline_ranking = fallback_order
        result["ai_scores_by_run_id"] = {}
    ...
    # v36.33: run_final_pass moved to the early position before pipeline_ranking
    # was built (see earlier block). The result is already in result["final_pass"]
    # by this point. The late call site here has been removed. We retain the
    # acceptable_drafts_in_rank_order list because Stage G's run_line_edit_pass
    # below still consumes it (the runners-up serve as the same-beat graft
    # donor pool for any AI-tell flags Stage G surfaces).
    try:
    ...
    if not LOCAL_SCORER_AVAILABLE:
        # Fallback: v25 behavior — literary winner ships ungated.
        ...
        try:
            ...
            if not pre_text.strip():
                ...
                if pre_score >= SCORE_SHIP:
                    # Ship as-is.
                    result["local_score"]["decision"] = "SHIP"
                    result["local_score"]["shipped_text_source"] = "final_text"
                    result["shipped"] = True
                elif pre_score >= SCORE_STAGE_G_LOW:
                    # Invoke Stage G explicitly, then re-score.
                    # v36.28: defer the write of result["final_text_lineedited"]
                    # until the decision is known. If Stage G hurts or fails to
                    # improve the score, the edits are discarded and the iter
                    # offers the pre-G text as its best-observed output.
                    ...
                    try:
                        ...
                        result["line_edit"] = {
                            "ran": False,
                            "enabled": True,
                            "error": f"Stage G crashed under v36.30 invoke: {e}",
                    ...
                    # v36.28: three-way decision on the (pre, post) delta.
                    if post_score >= SCORE_SHIP:
                        # Stage G cleared the bar. Commit edits, ship lineedited.
                        if did_change:
                        ...
                        # Stage G helped but did not clear the bar. Commit
                        # the edits as the iter's best-observed text; continue
                        # the regen loop looking for a >= SHIP draft.
                        if did_change:
                            result["final_text_lineedited"] = edited_text_candidate
                        ...
                        # Stage G did not improve the score (post <= pre).
                        # Discard the edits. The iter's best-observed text is
                        # the pre-G final_text. Leaving final_text_lineedited
                        # unset keeps Stage F prediction, file labels, the
                        # debug-report scorer, the display text, and the
                        # GitHub push list naturally pointed at the pre-G
                        # text. The line_edit dict and the pre/post scores
                        # are preserved in the result for diagnostic value.
                        result["local_score"]["decision"] = "STAGE_G_REVERT_REGEN"
                        result["local_score"]["shipped_text_source"] = "final_text"
                        result["needs_regen"] = True
                else:
                    # Too far off-target — regen, no Stage G attempt.
                    result["local_score"]["decision"] = "REGEN"
                    result["needs_regen"] = True
        except Exception as e:
            # Local scorer crashed mid-flight. Fall back to v25 behavior:
            # ship the literary winner; do not loop. Record the error.
            # v36.27: capture the full traceback, not just str(e), so the
            # summary writer can surface the actual failure point.
...
def write_batch_summary(
    ...
    if not ls:
        ...
        lines.append("Local scorer UNAVAILABLE — fell back to v25 ship-the-literary-")
        lines.append("winner behavior. No AI-score gate, no Stage G, no regen loop.")
        if ls.get("reason"):
        ...
        if ls.get("reason"):
            lines.append(
                "Local scorer RUNTIME ERROR — fell back to v25 ship-the-"
                "literary-winner"
            )
            lines.append(
                "behavior. The modules loaded at startup but score_text() "
                "crashed during"
            )
            lines.append(
...
if LOCAL_SCORER_AVAILABLE:
    ...
    st.error(
        ...
        f"behavior reverts to v25 (ship literary winner ungated, "
...
# to v25 ship-the-literary-winner behavior with decision=SKIP.
if auto_key:
            ...
            if ls or regen_hist:
                with st.container(border=True):
                    ...
                    if not ls.get("available", False) and ls.get("reason"):
                        st.warning(
                            f"Local scorer unavailable — fell back to v25 "
                            f"ship-the-literary-winner behavior. Reason: "
                            f"{ls.get('reason')}"
                        )
                        ...
                        st.error(
                            ...
                            "the run. Fell back to v25 ship-the-literary-"
                            "winner behavior. No AI-score gate, no Stage G, "
                            "no regen loop."
                        )
                        with st.expander(
