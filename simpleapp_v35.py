...
v35 is the post-subtraction state of the bare-drafter experiment. The v34
SYSTEM_PROMPT discipline block (six numbered drafting rules: em-dash floor,
passage pattern budget, past-perfect cap, templated emotion echo, repeated
short fragment limit, long-sentence variance) is removed and SYSTEM_PROMPT
returns to the v33-baseline two-line form: "Follow the user's instructions
exactly. Do not add commentary, headers, or meta-text to your response."
Rationale: empirical pattern check on the v34 chapter showed the model
violated four of the six explicit rules (zero em-dashes against a 4-em-dash
floor, parallel-negation pair present, past-perfect rate worsened to 8.0
per 1k, templated echo still present) yet the chapter scored 82 — better
than v33. Whatever drove the gain wasn't compliance with the new rules,
and the v34 band map showed 11 of 18 orange runs were short procedural
sentences with no surface pattern at all, suggesting the rule block was
forcing a flatter register and the rules block wasn't catching those
runs anyway. v35 tests whether 82 holds, drops, or improves with the
discipline block removed. Diagnostic experiment, not a final architecture.
No scanner logic, no detector code, no gate thresholds touched. Strictly
prompt-side reversion.
...
Your job is to identify every sentence, clause, OR multi-sentence run in the runners-up that could usefully replace a counterpart in TOP 1. This is a wide-net identification pass. Do not commit yet. Commit decisions happen in the next step.
...
- RUN: replace a multi-sentence span (a staccato run, an anaphoric cluster, or an aphoristic couplet) with a SINGLE flowing donor sentence. RUN grafts are the only legitimate way to repair the staccato/anaphora/couplet flag types — those flag the whole run, not its parts. The donor for a RUN graft is one sentence; the TOP1_TEXT is the verbatim multi-sentence span flagged.
...
- For UNIT=run grafts: the donor is itself part of a staccato run, an anaphoric cluster, or an aphoristic couplet in the donor draft (it must be a SINGLE flowing sentence, not a fragment)
...
def _donor_sentence_is_clean(donor_sentence: str) -> bool:
    ...
    if _is_aphorism(donor_sentence):
        return False
    # Note: staccato runs and anaphoric escalation are multi-sentence
    # properties — they cannot be evaluated against a single isolated
    # donor sentence. Run-level filtering happens in the grafter when
    # it composes a donor passage; see _donor_passage_is_clean below.
    return True
