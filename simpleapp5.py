...
When you identify the top contender, also name the specific scene-level beats where it is strongest. Use concrete labels like "opening," "chandler scene," "soldier scene," "closing line" — the evaluator downstream needs to know which beats carry the strength so they can be grafted if needed.

OUTPUT FORMAT

For each draft, write a brief paragraph (2-4 sentences) citing a specific passage.

Then a comparison paragraph naming the top 2-3 contenders and why the top one edges the others. In that paragraph, name the specific beats where the top draft is strongest.
...
# Models that reject the `temperature` parameter with a 400 error.
# Claude Opus 4.7 deprecated sampling parameters entirely; requests that
# include temperature, top_p, or top_k are rejected at the API. Adding
# a model here will cause the app to strip temperature from its kwargs.
MODELS_NO_TEMPERATURE = ("claude-opus-4-7",)


def model_accepts_temperature(model: str) -> bool:
    """Return False if this model rejects the `temperature` API parameter."""
    if not model:
        return True
    return not any(model.startswith(m) for m in MODELS_NO_TEMPERATURE)


def build_create_kwargs(model: str, temperature: float | None, **rest) -> dict:
    """Assemble kwargs for client.messages.create, omitting temperature when
    the model does not accept it. Callers pass the temperature they would
    use; this helper drops it silently if the model rejects it, and keeps
    everything else (max_tokens, system, messages) intact."""
    kwargs = {"model": model, **rest}
...
def generate_chapter(client, model: str, temperature: float, message_blocks: list) -> str:
    resp = client.messages.create(**build_create_kwargs(
        model=model,
...
def evaluate_drafts_with_anthropic(client, model: str, drafts: list) -> dict:
    n = len(drafts)
...
def run_compliance_pipeline(
    ...
    lit = evaluate_drafts_with_anthropic(client, eval_model, drafts)
    ...
    # --- Stage D: Graft ---
    # Defensive: if graft_base_run_id or graft_donor_run_id is empty or not in
    # drafts (can happen if the rubric evaluator's parse returned nothing
    # identifiable), fall back to shipping the literary winner instead of
    # raising StopIteration and killing the whole pipeline silently.
    base_draft = next(
        (d for d in drafts if d["run_id"] == result["graft_base_run_id"]), None
    )
    donor_draft = next(
        (d for d in drafts if d["run_id"] == result["graft_donor_run_id"]), None
    )
    if base_draft is None or donor_draft is None:
        result["pipeline_path"] = "graft_skipped_missing_ids"
        final_text = next(d["text"] for d in drafts if d["run_id"] == lit["winner_run_id"])
        result["final_text"] = final_text
        final_path = FINAL_DIR / f"FINAL_{batch_stub}_graft_skipped.txt"
        save_text(final_path, final_text)
        result["final_path"] = str(final_path)
        return result

    base_text = base_draft["text"]
    donor_text = donor_draft["text"]
...
st.caption("Generate · Scan · Evaluate · Graft · Ship")
...
with st.sidebar:
    ...
    # Parse tolerantly: accept commas, periods, semicolons, or whitespace as
    # separators. Distinguish a decimal-point period from a separator-period
    # by looking at what surrounds it: "0.9. 1.0" has the pattern "digit . space"
    # which is a separator, while "0.9" has "digit . digit" which is a decimal.
    normalized = re.sub(r"(?<=\d)\.\s+", ",", temps_input)  # "0.9. 1.0" -> "0.9,1.0"
    normalized = re.sub(r"[;\s]+", ",", normalized)         # whitespace/semis -> commas
    try:
        temperatures = [float(t) for t in normalized.split(",") if t.strip()]
        if not temperatures:
            raise ValueError("empty")
    except ValueError:
        temperatures = [0.7]
        st.warning(
            f"Could not parse temperatures from {temps_input!r}. Using 0.7. "
            "Use commas: `0.7, 0.9, 1.0`"
        )

    # Warn the user if the chosen generation model rejects temperature entirely.
    # The app will strip the parameter before sending, but the user should know
    # that temperature-as-experimental-variable is not operative for this model.
    if not model_accepts_temperature(gen_model):
        st.info(
            f"Note: `{gen_model}` does not accept the temperature parameter. "
            f"The app will strip it from API calls. All runs in this batch "
            f"will use the model's default sampling regardless of the values "
            f"entered above."
        )
...
with left_col:
    ...
    if st.button("Generate", type="primary", disabled=not api_key or total_runs == 0):
        ...
        if problems:
            ...
            progress.empty()
            status.success(f"Done. {run_count} drafts generated and scanned.")
...
with right_col:
    ...
    if df.empty:
        ...
        # --- Run pipeline ---
        st.markdown("---")
        st.subheader("Run pipeline on most recent batch")
        st.caption(
            "Scans → literary evaluation → conditional graft → verification → ship. "
            "The app decides which path based on the scan results."
        )

        max_eval = min(25, len(df))
        last_batch = st.session_state.get("last_batch_size", None)
        default_eval = min(last_batch, max_eval) if last_batch else min(10, max_eval)
        eval_count = st.slider(
            "Drafts to include in pipeline (most recent N)",
            2, max_eval, default_eval,
        )

        if st.button("Run full pipeline", type="primary", disabled=not api_key):
            if "Outline" not in doc_uploads:
                st.warning(
                    "No outline uploaded in this session. The pipeline will still run, "
                    "but if the literary winner is mechanically dirty AND no other drafts "
                    "are clean, the rubric-fallback step will be skipped."
                )
            ...
            for _, row in batch_df.iterrows():
                ...
                drafts.append({"run_id": run_id, "text": text})

                # Pull scan from CSV or recompute if missing
                if str(row.get("scan_hard_cap_pass", "")).strip() not in ("", "nan"):
                    scan_by_run_id[run_id] = {
                        k: row.get(k) for k in row.index if k.startswith("scan_")
                    }
                    # Coerce types
                    sr = scan_by_run_id[run_id]
                    for int_k in [
                        "scan_the_way_count", "scan_periphrastic_count",
                        "scan_not_but_count", "scan_em_dash_count",
                        "scan_emotion_naming_count", "scan_semicolons",
                        "scan_colons", "scan_parens",
                    ]:
                        try:
                            sr[int_k] = int(float(sr.get(int_k, 0) or 0))
                        except (ValueError, TypeError):
                            sr[int_k] = 0
                    for float_k in [
                        "scan_em_dash_per_1k", "scan_avg_sentence_len",
                        "scan_long_sentences_pct", "scan_fragments_pct",
                    ]:
                        try:
                            sr[float_k] = float(sr.get(float_k, 0) or 0)
                        except (ValueError, TypeError):
                            sr[float_k] = 0.0
                    sr["scan_hard_cap_pass"] = str(sr.get("scan_hard_cap_pass", "")).lower() in ("true", "1")
                else:
                    # Re-scan on the fly for legacy rows
                    scan_by_run_id[run_id] = scan_draft(text)

            if len(drafts) < 2:
                st.error("Need at least 2 readable drafts for the pipeline.")
            else:
                outline_text = doc_uploads.get("Outline", "")
                temps_used = sorted(set(float(x) for x in batch_df["temperature"].dropna()))
                prompts_used = sorted(set(int(x) for x in batch_df["prompt_id"].dropna()))
                ...
                with st.spinner("Running pipeline..."):
                    try:
                        ...
                        batch_run_id_list = [d["run_id"] for d in drafts]
                        update_records_bulk(csv_path, batch_run_id_list, {
                            "is_winner": False,
                        ...
                        if result["pipeline_path"] == "direct_ship":
                            ...
                            if result["graft_verified"]:
                                ...
                                st.warning(
                                    ...
                                    f"See FAILED_GRAFT file in final_deliverables/."
                                )
                        ...
                        st.markdown("**Files produced:**")
                        st.write(f"Final: `{result['final_path']}`")
                        ...
                        # Only rerun on SUCCESS — inside the try block.
                        # On failure we want the error banner to stay on
                        # screen, which means no rerun.
                        st.rerun()

                    except Exception as e:
                        # Persist the traceback to disk so the error survives
                        # Streamlit's rerun-on-error behavior. Without this,
                        # st.rerun() (or any page refresh) wipes the red banner
                        # before the user can read it, and the exception is
                        # lost entirely. The file also gives us something to
                        # diff against on the next failure.
                        import traceback
                        tb_text = traceback.format_exc()
                        try:
                            FINAL_DIR.mkdir(parents=True, exist_ok=True)
                            err_ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                            err_path = FINAL_DIR / f"PIPELINE_ERROR_{err_ts}.txt"
                            err_path.write_text(
                                f"Pipeline failed at {datetime.now().isoformat()}\n"
                            ...
                            st.error(
                                f"Pipeline failed: {e}\n\n"
                                f"Full traceback saved to {err_path}. "
                                f"Download the runs zip to retrieve it."
                            )
                        except Exception:
                            st.error(f"Pipeline failed: {e}")
                        st.code(tb_text)
                        # No rerun here. The banner and traceback stay visible
