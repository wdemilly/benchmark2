...
The pipeline answers three questions in order:
  ...
  Q3. Two graft pathways, both always active:

      Type A — FLAG REPAIR. If TOP 1 carries a flagged construction at a
      beat where a runner-up has a clean same-beat donor, transplant the
      clean sentence in. This repairs mechanical tells.

      Type B — QUALITY UPGRADE. If a runner-up sentence at the same beat
      is genuinely better-written — sharper image, more specific detail,
      stronger wit, more distinctive voice, better dialogue — transplant
      it regardless of whether TOP 1's sentence is flagged. The donor
      must still be clean of hard-cap patterns and must do the same
      narrative work. This is for standout sentences, not marginal
      improvements.

      Both pathways require same-narrative-work and donor cleanliness.
...
Your job is to find sentences in the runners-up that should replace their counterparts in TOP 1. There are two reasons to graft — both are always active.

TYPE A — FLAG REPAIR. TOP 1's sentence carries a flagged construction (listed below) and a runner-up has a clean version at the same beat.

TYPE B — QUALITY UPGRADE. A runner-up sentence at the same beat is genuinely better-written than TOP 1's version — a sharper image, a more specific physical detail, a stronger line of wit, a more distinctive piece of interior voice, a better piece of dialogue. The runner-up sentence is the one a reader would underline. This is for standout sentences that outperform the winner at a specific beat, not marginal improvements in phrasing or cadence.

BOTH TYPES require all of the following:

1. SAME BEAT, SAME NARRATIVE WORK. The donor sentence sits at the same story beat and does the same job — conveys the same information, moves the scene the same way, reveals the same character note. Transplanting it must not break continuity with the sentences before and after it in TOP 1.

2. DONOR IS CLEAN. The donor sentence contains ZERO of the flaggable constructions below. A donor that introduces a tell is disqualified even if it is otherwise brilliant. Check the donor against every item before accepting it.

   Flaggable constructions:
   ...
   - "not X but Y" negation pivots in narration or interior thought (dialogue is permitted)
...
For TYPE A specifically: the TOP 1 sentence must carry one of the flagged constructions. Use the scanner-flagged passages below as your starting list.

For TYPE B specifically: the TOP 1 sentence does NOT need to be flagged. The graft fires because the runner-up version is meaningfully better prose. When proposing a Type B graft, name what makes the donor sentence better — do not just say it is "stronger" or "sharper."

If no qualifying grafts exist under either type, return NO_GRAFT.

Do NOT cap your list artificially. Return as many qualifying grafts as you find under both types.
...
REASON: [For Type A: name which flagged construction this replaces and confirm donor is clean. For Type B: name specifically what makes the donor sentence better and confirm donor is clean of all flags.]
...
Or if no grafts qualify under either type:
...
Do NOT force grafts. Every graft must pass the same-beat and clean-donor tests. Nothing after the last GRAFT entry or NO_GRAFT."""
...
def parse_line_grafts(raw: str) -> list:
    ...
    type_pat = re.compile(r"TYPE:\s*([AB])")
    ...
    for section in sections[1:]:
        ...
        type_m = type_pat.search(section)
...
def run_line_graft_experiment(
    ...
    """Identify runner-up sentences that improve the winner, then apply via
    deterministic string replacement.

    Two graft pathways:
      Type A — Flag Repair: winner sentence carries a hard-cap violation,
               runner-up has a clean version at the same beat.
      Type B — Quality Upgrade: runner-up sentence at the same beat is
               genuinely better-written, regardless of whether the winner's
               sentence is flagged. Donor must still be clean.
...
def write_batch_summary(
    ...
    if not lg:
        ...
        for i, g in enumerate(lg["grafts"], 1):
            ...
            lines.append(f"  Graft {i} — Type {gtype} ({type_label}) from Draft {g['source_draft']}:")
