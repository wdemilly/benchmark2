...
Your job is to find sentences in Drafts 2–{N} that would improve TOP 1's DETECTOR PROFILE — the rhythmic and structural signature that AI-detection tools read — while preserving what the chapter is doing at that beat.

You are NOT looking for sentences that are more beautiful, more elegant, more literary, or more emotionally resonant than TOP 1's version. You are looking for sentences that do the same narrative work as TOP 1's version AND have a materially different structural or rhythmic profile.

A candidate sentence qualifies ONLY if ALL of the following hold:

1. SAME NARRATIVE WORK. The donor sentence sits at the same beat as a sentence in TOP 1 and does the same thing — conveys the same information, moves the scene the same way, reveals the same character note. Transplanting it must not break continuity with the sentences around it in TOP 1.

2. DIFFERENT STRUCTURAL OR RHYTHMIC PROFILE. The donor sentence differs from TOP 1's version in at least one of these dimensions:
   - Materially different sentence length (meaningfully shorter or longer)
   - Different clause structure (subordinate where TOP 1 has coordinate, or vice versa)
   - A fragment where TOP 1 has a full sentence, or a full sentence where TOP 1 has a fragment
   - A period break where TOP 1 uses an em dash, semicolon, or colon
   - An unbalanced list where TOP 1 has a balanced triad (or vice versa)
   - A concrete, physical detail where TOP 1 uses an abstraction or summary
   - A plain statement where TOP 1 uses a "the way X" / "as though" / "not X but Y" construction
   - A character observation rendered as action or speech where TOP 1 renders it as interior narration

   If the donor sentence is just a more elegant way of saying the same thing in the same shape, it does NOT qualify. Elegance is not the target. Rhythmic and structural difference is.

3. DONOR IS CLEAN. The donor sentence itself contains ZERO of these hard-cap patterns: "the way X" observational framing (e.g., "the way he held the spoon"), periphrastic "as though he were" / "in the manner of" observational framing, "not X but Y" negation pivots in narration, named emotions in narration (e.g., "a wave of sadness"). If the donor carries any of these, it is disqualified — transplanting it would only move the problem.

Do NOT cap your list artificially. Return as many qualifying sentences as you find; return zero if none qualify.

KNOWN WEAK SPOTS IN TOP 1

The mechanical scanner flagged these passages in TOP 1 as carrying hard-cap violations. These are the places where a clean donor replacement would help the detector profile most. Check these first.
...
REASON: [one sentence naming the structural or rhythmic difference, not the literary improvement]

GRAFT 2:
SOURCE: Draft [number]
<replace>[exact text from TOP 1 to remove]</replace>
<with>[exact text from the runner-up to insert]</with>
REASON: [one sentence naming the structural or rhythmic difference]
...
Do NOT force grafts. Return NO_GRAFT if no runner-up sentence meets all three conditions. Nothing after the last GRAFT entry or NO_GRAFT."""
