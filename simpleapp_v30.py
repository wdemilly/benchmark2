...
# v30: word-count overshoot — the drafter is told to overshoot the
# outline's stated target by this percentage, so the line-edit pass
# (which now includes Type C deletions) lands the chapter at target
# after cuts. Calibrated against the v29 chapter's red-band word
# share (~17–18%): deleting roughly half of those reds lands within
# this buffer.
STAGE_G_OVERSHOOT_PCT = 0.05  # 5%
...
def _build_word_count_overshoot_block(outline_text: str) -> str:
    ...
    drafter to overshoot the outline's word-count target by ~5% so the
    downstream line-edit pass (which now includes Type C deletions) lands
    the chapter at target after cuts. Returns an empty string if the
    outline has no detectable word-count target.
    """
