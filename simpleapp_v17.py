...
STAGE_F_RIDGE_LAMBDA = 1.0           # L2 regularization
...
# replacement) tuples. Patterns are case-insensitive. Replacements are
# applied in order. The a/an article repair runs afterward on the whole
# edited sentence.
#
# To add a word: append an entry. Start with the deletion pattern you want;
# if the word is context-sensitive, write a narrower pattern so false
...
#
# Fits a 15-feature ridge regression against labeled_corpus.json (doc text +
# known Originality human-score). At inference, extracts the same 15 features
# from the shipped final_text and returns a predicted integer score 0–100
# plus a recommendation band (SHIP / RECONSIDER / REGENERATE).
#
...
# or out of band.
#
# Validation (20-row sample vs this corpus family): ridge MAE ~11.75,
# Pearson r ~0.79; at pred≥88 gate, precision at real≥90 ran 60–100% in
# LOO on subsets of the corpus. Numbers will shift with corpus size; the
# ridge refits on every app startup so additions to labeled_corpus.json
# take effect on the next deploy.
# ============================================================================


# 15 style features matching the v16_predictor spec:
#   1-5   sentence-length stats (mean, std, max, min, median)
#   6-9   length-band percentages (ultra-short, short, medium, long)
#   10    paragraph-length mean (words)
#   11-15 per-1k rates: em-dashes, semicolons, colons, "the-way", "not-X-but-Y"
STAGE_F_FEATURE_NAMES = [
    "sent_len_mean", "sent_len_std", "sent_len_max", "sent_len_min", "sent_len_median",
    "pct_ultrashort", "pct_short", "pct_medium", "pct_long",
    "para_len_mean",
    "em_dash_per_1k", "semicolon_per_1k", "colon_per_1k",
    "the_way_per_1k", "not_but_per_1k",
]
...
def _stage_f_paragraph_lengths(text: str) -> list:
    paras = [p.strip() for p in re.split(r"\n\s*\n", text) if p.strip()]
    return [len(re.findall(r"\b[\w']+\b", p)) for p in paras]


def stage_f_extract_features(text: str) -> np.ndarray:
    """Return the 15-feature vector for a draft. Order matches STAGE_F_FEATURE_NAMES."""
    words = re.findall(r"\b[\w']+\b", text)
    wc = max(len(words), 1)

    sl = _stage_f_sentence_lengths(text)
    if not sl:
        sl = [0]
    sl_arr = np.asarray(sl, dtype=float)

    total_sents = max(len(sl), 1)
    ultrashort = sum(1 for x in sl if 1 <= x <= 5)
    short_s = sum(1 for x in sl if 6 <= x <= 12)
    medium = sum(1 for x in sl if 13 <= x <= 25)
    long_s = sum(1 for x in sl if x > 40)

    pl = _stage_f_paragraph_lengths(text) or [0]
    pl_arr = np.asarray(pl, dtype=float)

    em_dashes = text.count("\u2014")
    semicolons = text.count(";")
    colons = len(re.findall(r"(?<!\d):(?!\d)", text))
    the_way = len(THE_WAY_PATTERN.findall(text))
    not_but = 0
    for m in NOT_BUT_PATTERN.finditer(text):
        before = text[: m.start()]
        normalized = before.replace("\u201c", '"').replace("\u201d", '"')
    ...
    feats = [
        ...
        float(sl_arr.max()),
        float(sl_arr.min()),
        float(np.median(sl_arr)),
        100.0 * ultrashort / total_sents,
        100.0 * short_s / total_sents,
        100.0 * medium / total_sents,
        100.0 * long_s / total_sents,
        float(pl_arr.mean()),
        em_dashes / wc * 1000.0,
        semicolons / wc * 1000.0,
        colons / wc * 1000.0,
        the_way / wc * 1000.0,
        not_but / wc * 1000.0,
    ]
...
def _stage_f_loo_mae(X: np.ndarray, y: np.ndarray, lam: float) -> float:
    """Leave-one-out MAE for the ridge. n refits, each O(p³) — trivial at p=16."""
    n = X.shape[0]
    errs = []
    idx = np.arange(n)
    for i in range(n):
        mask = idx != i
        beta = _stage_f_fit_ridge(X[mask], y[mask], lam)
        pred = float(X[i] @ beta)
...
def stage_f_load_predictor(corpus_path_str: str, lam: float) -> dict:
    ...
    loo_mae = _stage_f_loo_mae(X_full, y, lam)
...
def stage_f_predict(text: str, predictor: dict) -> dict:
    """Predict Originality human-score for `text` using the fitted ridge.

    Returns a dict with:
      available: bool
      reason: str — "" if available, else explanation
      predicted_score: int — clamped 0–100
      band: str — SHIP / RECONSIDER / REGENERATE / UNAVAILABLE
      n_train: int
      loo_mae: float
    ...
    mu = predictor["mu"]; sigma = predictor["sigma"]; beta = predictor["beta"]
    x_norm = (feats - mu) / sigma
    x_full = np.concatenate([[1.0], x_norm])
    raw = float(x_full @ beta)
    ...
    return {
        ...
        "features": {name: round(float(v), 3) for name, v in zip(STAGE_F_FEATURE_NAMES, feats)},
    }
...
def apply_deletion_heuristic(sentence: str, ai_tell_words: dict) -> tuple:
    ...
    (modified_sentence, list_of_edits_made)."""
    edits = []
    result = sentence
    for word, patterns in ai_tell_words.items():
...
def run_pipeline(
    ...
    # --- Stage G: line-edit pass (mechanical copyedit + AI-tell handling) ---
    # Runs on final_text. G1 applies a strict punctuation-only mechanical
    # copyedit with a word-sequence invariant check. G2 scans the result for
    # AI-tell constructions (module-level AI_TELL_WORDS). G3 resolves each
    # flagged sentence with a three-behavior cascade: same-beat graft from a
    # runner-up, deletion with a/an repair, or flag-for-rewrite. Original
    # final file is preserved; edited version ships as a sibling file.
    try:
        ...
        result["line_edit"] = le
        # If the line-edit pass produced a cleaned text, use it as the basis
        # for Stage F's prediction. The FINAL file on disk is untouched; the
        # LINEEDITED file becomes the ship candidate for downstream scoring.
    ...
    # --- Stage F: ridge predictor of Originality human-score (advisory) ---
    # Runs on the final_text (or on the line-edited text if Stage G produced
    # one). Does not gate shipping. The predicted score is written to the
    # summary alongside the recommendation band. If the labeled corpus is
    # missing or too small, Stage F reports UNAVAILABLE and the rest of the
    # pipeline is unchanged.
    try:
        predictor = stage_f_load_predictor(str(LABELED_CORPUS_PATH), STAGE_F_RIDGE_LAMBDA)
        # Prefer the line-edited text if Stage G produced one
...
def write_batch_summary(
    ...
    if not sf.get("available"):
        ...
        lines.append(f"Corpus: {n_train} labeled docs · LOO MAE {loo_mae}")
        lines.append("Note: ridge regression on 15 style features. Advisory only — "
