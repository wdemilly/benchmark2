    ...
    with left:
        ...
        if run_selected:
            if not api_key:
                ...
                for idx, prompt_obj in enumerate(selected_prompts, start=1):
                    run_id = datetime.now().strftime("%Y%m%d_%H%M%S") + f"_{prompt_obj['id']:02d}"
                    ...
                    micro_prompt_path = run_dir / f"{run_id}_micro_prompt.txt"
                    meta_path = run_dir / f"{run_id}_meta.json"

                    try:
                        status.write(f"Running prompt {prompt_obj['id']} ({idx} of {len(selected_prompts)})...")
                    ...
                    progress.progress(idx / len(selected_prompts))
                    time.sleep(0.1)
    ...
    with right:
        ...
        if df.empty:
            ...
            with st.form("score_form"):
                ...
                originality_score = st.text_input("Originality score", value="" if pd.isna(current.get("originality_score")) else str(current.get("originality_score")))
                manual_rating = st.selectbox("Manual rating", ["", "strong", "decent", "weak"], index=["", "strong", "decent", "weak"].index(str(current.get("manual_rating", "") or "")) if str(current.get("manual_rating", "") or "") in ["", "strong", "decent", "weak"] else 0)
                ...
                if submitted:
                    ...
                    update_record(csv_path, selected_run, {
            ...
            st.download_button("Download outputs + CSV", data=zip_bytes, file_name="micro_prompt_runs_export.zip", mime="application/zip")
