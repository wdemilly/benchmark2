...
def scan_draft(text: str) -> dict:
    ...
    hard_cap_pass = (
        ...
        # v32 hard caps — Originality-orange cadences scan_draft missed in v31
        and len(triple_fragment_violations) == 0
        and len(triple_adj_stack_violations) == 0
        and len(anaphoric_relaxed_violations) == 0
        and len(summative_closer_violations) == 0
        and len(which_stack_violations) == 0
        and len(body_inventory_violations) == 0
        and len(participle_cascade_violations) == 0
        and len(tense_shift_violations) == 0
    )
    return {
