...
def run_pipeline(
    ...
    if len(acceptable_drafts_in_rank_order) >= 2:
        # v36.32: wrap final_pass in try/except for the same reason as the
        # graft wrap above — final_pass also makes unprotected API calls and
        # parses model output. A crash here used to propagate out and abort
        # the rest of the pipeline (Stage G line-edit, local-scorer flow,
        # Stage F prediction). Treat a final-pass failure as "no final pass
        # output" and continue.
        try:
            ...
            result["final_pass"] = {
                "error": (
                    f"run_final_pass crashed under v36.32 wrap: "
