...
v36.26 REPLACES Stage G's invocation model. The Stage G machinery
...
AI-detection signal. In v36.26 STAGE_G_ENABLED is False at module level,
...
v36.26 also wraps the main "Generate & Evaluate" button in a regeneration
...
fails to import, v36.26 logs the reason and falls back to v25's shipping
...
# v36.26 additions: local scorer stack. These four modules must sit beside
# simpleapp for the new score-driven Stage G flow. If any import fails,
# LOCAL_SCORER_AVAILABLE stays False and v36.26 falls back to v25's
...
# v36.26: STAGE_G_ENABLED is False at module level. The existing
...
# v36.26 local-scorer-driven loop constants.
...
def run_line_edit_pass(
    ...
    Args:
        ...
        force_enable: v36.26 — when True, bypass the module-level
                      STAGE_G_ENABLED flag. The v36.26 local-scorer flow
...
def run_pipeline(
    ...
    # ========================================================================
    # v36.26: local-scorer-driven Stage G flow.
    #
    ...
    result["local_score"] = {
        ...
        "post_g_score": None,
        "post_g_details": {},
        "decision": "",          # SHIP / STAGE_G_THEN_SHIP / STAGE_G_THEN_REGEN / REGEN / SKIP
    ...
    if not LOCAL_SCORER_AVAILABLE:
        ...
        try:
                ...
                if pre_score >= SCORE_SHIP:
                    ...
                    try:
                        ...
                        result["line_edit"] = {
                            "ran": False,
                            "enabled": True,
                            "error": f"Stage G crashed under v36.26 invoke: {e}",
                    ...
                    if post_score >= SCORE_SHIP:
                        ...
                        result["local_score"]["decision"] = "STAGE_G_THEN_REGEN"
                        result["needs_regen"] = True
                else:
            ...
            result["local_score"]["reason"] = (
                f"local scorer crashed during run_pipeline: {e}"
            )
            result["local_score"]["decision"] = "SKIP"
            result["local_score"]["shipped_text_source"] = "final_text"
            result["shipped"] = True
...
def write_batch_summary(
    ...
    lines.append("")
    lines.append("=" * 60)
    lines.append("v36.26 — LOCAL-SCORER-DRIVEN STAGE G FLOW")
    lines.append("=" * 60)
    ls = pipeline_result.get("local_score") or {}
    if not ls:
        lines.append("(no local_score block on pipeline_result — pre-v36.26 result?)")
        ...
        lines.append("")
        lines.append("To engage v36.26, ensure band_classifier.py, "
                     "extended_band_features.py,")
        lines.append("local_scorer.py, and calibration.json are committed to "
                     "the repo")
        lines.append("alongside simpleapp_v36_26.py.")
    else:
...
# ============================================================================
# v36.26 startup banner — local-scorer load status.
#
...
# rather than only at end-of-batch when nothing of v36.26 ran.
# ============================================================================
if LOCAL_SCORER_AVAILABLE:
    ...
    try:
        ...
        st.warning(
            f"v36.26 local scorer modules loaded, but reading "
            f"calibration.json metrics failed: {_banner_exc}"
        )
    if _banner_calibrated:
        st.success(
            f"v36.26 ACTIVE · local scorer loaded · "
        ...
        st.warning(
            f"v36.26 PARTIALLY ACTIVE · local scorer modules loaded "
    ...
    st.error(
        f"v36.26 FALLBACK MODE · local scorer modules NOT loaded · "
        ...
        f"GitHub repo in the same directory as simpleapp_v36_26.py."
    )
...
with left_col:
    ...
    if st.button("Generate & Evaluate", type="primary", disabled=not api_key or total_runs == 0):
        ...
        if problems:
            ...
            # ================================================================
            # v36.26 regeneration loop. Wraps generate → pipeline → score.
            ...
            for iter_idx in range(loop_iterations):
                ...
                if len(batch_drafts) >= 1:
                    ...
                    try:
                        ...
                        # ------------------------------------------------------
                        # v36.26: assess this iteration's result and decide
                        # whether to break, update best, or bump temps and loop.
                        # ------------------------------------------------------
                        ls = result.get("local_score", {})
                        pre_s = ls.get("pre_g_score")
                        post_s = ls.get("post_g_score")
                        iter_score = max(
                            float(pre_s) if pre_s is not None else -1.0,
                            float(post_s) if post_s is not None else -1.0,
                        )
                    ...
                    break
            # ================================================================
            # End of v36.26 regen loop. Surface the best result observed.
            # ================================================================
...
with right_col:
    ...
    if df.empty:
        ...
        if result:
            ...
            # --- v36.26 status block: local-scorer + regen loop summary ---
            ls = result.get("local_score") or {}
            regen_hist = result.get("regen_history") or []
            if ls or regen_hist:
                with st.container(border=True):
                    st.markdown("**v36.26 local-scorer flow**")
