...
v36.25 REPLACES Stage G's invocation model. The Stage G machinery
...
AI-detection signal. In v36.25 STAGE_G_ENABLED is False at module level,
...
v36.25 also wraps the main "Generate & Evaluate" button in a regeneration
...
fails to import, v36.25 logs the reason and falls back to v25's shipping
...
# v36.25 additions: local scorer stack. These four modules must sit beside
# simpleapp for the new score-driven Stage G flow. If any import fails,
# LOCAL_SCORER_AVAILABLE stays False and v36.25 falls back to v25's
...
# v36.25: STAGE_G_ENABLED is False at module level. The existing
...
# v36.25 local-scorer-driven loop constants.
...
def run_line_edit_pass(
    ...
    Args:
        ...
        force_enable: v36.25 — when True, bypass the module-level
                      STAGE_G_ENABLED flag. The v36.25 local-scorer flow
...
def run_pipeline(
    ...
    # ========================================================================
    # v36.25: local-scorer-driven Stage G flow.
    #
                ...
                if pre_score >= SCORE_SHIP:
                    ...
                    try:
                        ...
                        result["line_edit"] = {
                            "ran": False,
                            "enabled": True,
                            "error": f"Stage G crashed under v36.25 invoke: {e}",
...
with left_col:
    ...
    if st.button("Generate & Evaluate", type="primary", disabled=not api_key or total_runs == 0):
        ...
        if problems:
            ...
            # ================================================================
            # v36.25 regeneration loop. Wraps generate → pipeline → score.
            ...
            for iter_idx in range(loop_iterations):
                ...
                if len(batch_drafts) >= 1:
                    ...
                    try:
                        ...
                        # ------------------------------------------------------
                        # v36.25: assess this iteration's result and decide
                    ...
                    break
            # ================================================================
            # End of v36.25 regen loop. Surface the best result observed.
            # ================================================================
...
with right_col:
    ...
    if df.empty:
        ...
        if result:
            ...
            # --- v36.25 status block: local-scorer + regen loop summary ---
            ls = result.get("local_score") or {}
            regen_hist = result.get("regen_history") or []
            if ls or regen_hist:
                with st.container(border=True):
                    st.markdown("**v36.25 local-scorer flow**")
